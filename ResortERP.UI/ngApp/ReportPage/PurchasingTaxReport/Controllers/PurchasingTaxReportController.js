'use strict';
app.controller('PurchasingTaxReportController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService', 'compBranchesService', 'costCentersService', 'PurchasingTaxReportService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, compBranchesService, costCentersService, PurchasingTaxReportService) {
    ...
    $scope.PurchasingTaxReportPageLoad = function () {
    ...
    $scope.GetSearchResult = function () {
        if ($scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            ...
            PurchasingTaxReportService.getReportData(companyBranches, startDate, endDate).then(function (results) {
                if (results != undefined && results.data != undefined && results.data.length > 0) {
                    $scope.ReportData = results.data;
    ...
    $scope.PdfPrintReport = function () {

        var x1 = "تقرير ضريبة المشتريات".split(" ").reverse().join(" ");
        pdfMake.fonts = {
            custom: {
            ...
            content: [
                {
                    ...
                    table: {
                        ...
                        headerRows: 1,
                        widths: [75, 45, 45, 60, 'auto', 'auto', 'auto', 'auto', 50, 'auto', 60, 30, 30, 30, 30, 25, 45, 'auto', 40, 35, 'auto', 'auto'],

                        //widths: [75, 45, 45, 60, 30, 30, 30, 30, 60, 30, 55, 30, 30, 30, 30, 30, 50, 40, 50, 50, 60, 50],
                        //margin: [400, 400, 10, 10],
                        body: $scope.createArr($scope.ReportData)
                    }
                }
            ],
            images: {
    ...
    $scope.createArr = function (list) {
        ...
        for (var i = 0; i < 22; i++) {
            ...
            arr[0][i].text = arr[0][i].text.split(" ").reverse().join(" ");
        }
            ...
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (key == "BILL_DATE") {
                        ...
                        arr[index][19] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "BILL_ABRV_AR") {
                        arr[index][18] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "ACC_AR_NAME") {
                        arr[index][17] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "ACC_CODE") {
                        arr[index][16] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "DeliveryPersonName") {
                        arr[index][15] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "TransTotalGold24") {
                        arr[index][14] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TransTotalGold22") {
                        arr[index][13] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TransTotalGold21") {
                        arr[index][12] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TransTotalGold18") {
                        arr[index][11] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalBeforeTax") {
                        arr[index][10] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalAllDisc") {
                        arr[index][9] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    //else if (key == "TotalMustPaid") {
                    //    arr[index][9] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    //}                   
                    else if (key == "TotalNotTaxableAmount") {
                        arr[index][8] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalMainVat") {
                        arr[index][7] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "MainVatValue") {
                        arr[index][6] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalZeroVat") {
                        arr[index][5] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "ZeroVatValue") {
                        arr[index][4] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalExemptOfTax") {
                        arr[index][3] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "ExemptOfTaxValue") {
                        arr[index][2] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalVatValue") {
                        arr[index][1] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalAfterTax") {
                        arr[index][0] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    j++;
                }
            }
