'use strict';
app.controller('AccountDetectingReportController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService', 'compBranchesService', 'costCentersService', 'AccountDetectingReportService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, compBranchesService, costCentersService, AccountDetectingReportService) {


    $scope.accountList = [];

    $scope.entryMasterList = [];

    $scope.cust_Acc_ID = null;

    $scope.startDate = "";
    $scope.endDate = "";

    $scope.show_print = false;
    $scope.var = { ACC_ID: null };
  
    $scope.UserBranches = [];
    $scope.dropdownSetting = {
        scrollable: true,
        scrollableHeight: '300px',
        checkBoxes: true,
        enableSearch: true,
        buttonClasses: 'btn btn-default btn-block',
        styleActive: true,
        smartButtonTextProvider: function (selectedItems) {
            if (selectedItems.length > 3) {
                return "تم اختيار " + selectedItems.length + ' فروع';
            }
            else if (selectedItems.length == 3) {
                return selectedItems[0].label + ', ' + selectedItems[1].label + ', ' + selectedItems[2].label;
            }
            else if (selectedItems.length == 2) {
                return selectedItems[0].label + ', ' + selectedItems[1].label;
            }
            else if (selectedItems.length == 1) {
                return selectedItems[0].label;
            }
            else {
                return 'اختار فرع';
            }
        }
    }


    $scope.AccountDetectingReportPageLoad = function () {

        //$scope.getAccountList();
        $scope.GetUserBranches();
        //$scope.getCostCentersList();

        $scope.var.ACC_ID = 775;
        $scope.startDate = new Date(2018, 8, 14);
        $scope.endDate = new Date();
    }

    $scope.GetUserBranches = function () {
        compBranchesService.getUserBranches().then(function (response) {
            if (response.data != undefined && response.data.length > 0) {
                angular.forEach(response.data, function (value, index) {
                    $scope.UserBranches.push({ id: value.COM_BRN_ID, label: value.COM_BRN_AR_NAME });
                });
                $scope.SelectedBranch = [$scope.UserBranches[0]];
            }
        }, function (error) { console.log(error.data.message); });
    };

    $scope.GetSearchResult = function () {
        if ($scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {

            var companyBranches = '';
            angular.forEach($scope.SelectedBranch, function (item, index) {
                companyBranches += item.id + ',';
            })
            companyBranches = companyBranches.substr(0, companyBranches.length - 1);

            var d1 = new Date($scope.startDate);

            var startDate = d1.getFullYear() + "" + ("0" + (d1.getMonth() + 1)).slice(-2) + "" + ("0" + d1.getDate()).slice(-2);

            var d2 = new Date($scope.endDate);

            var endDate = d2.getFullYear() + "" + ("0" + (d2.getMonth() + 1)).slice(-2) + "" + ("0" + (d2.getDate())).slice(-2);

            AccountDetectingReportService.getAccountReportData(companyBranches, startDate, endDate).then(function (results) {

                console.log(results.data);
                $scope.TotalSoldQuantity = 0, $scope.TotalSoldRealValue = 0, $scope.TotalPurchaseQuantity = 0, $scope.TotalPurchaseRealValue = 0;

                $scope.entryMasterList = $scope.CalculateTotalsAndAverages(results.data);

                $scope.show_print = true;
            });

        }

        else swal("عفوا", " لابد من البحث بالتاريخ والفرع", "error");

    }

    $scope.CalculateTotalsAndAverages = function (ReportData) {
        angular.forEach(ReportData, function (Row, Index) {
            $scope.TotalSoldQuantity = Row.TotalSoldQuantity = (parseFloat(Row.SalesActualItemWeightQTY == null || Row.SalesActualItemWeightQTY == undefined ? 0 : Row.SalesActualItemWeightQTY) + parseFloat($scope.TotalSoldQuantity)).toFixed(2);
            $scope.TotalSoldRealValue = Row.TotalSoldRealValue = $scope.TotalSoldRealValue = (parseFloat(Row.SalesTotalMustPaid == null || Row.SalesTotalMustPaid == undefined ? 0 : Row.SalesTotalMustPaid) + parseFloat($scope.TotalSoldRealValue)).toFixed(2);
            Row.GeneralAverageSoldPrice = (Row.TotalSoldRealValue == undefined || Row.TotalSoldRealValue == 0 || Row.TotalSoldQuantity == undefined || Row.TotalSoldQuantity == 0 ? 0 : Row.TotalSoldRealValue / Row.TotalSoldQuantity * 1000).toFixed(2);

            $scope.TotalPurchaseQuantity = Row.TotalPurchaseQuantity = (parseFloat(Row.PurchasingActualItemWeightQTY == null || Row.PurchasingActualItemWeightQTY == undefined ? 0 : Row.PurchasingActualItemWeightQTY) + parseFloat($scope.TotalPurchaseQuantity)).toFixed(2);
            $scope.TotalPurchaseRealValue = Row.TotalPurchaseRealValue = (parseFloat(Row.PurchasingTotalMustPaid == null || Row.PurchasingTotalMustPaid == undefined ? 0 : Row.PurchasingTotalMustPaid) + parseFloat($scope.TotalPurchaseRealValue)).toFixed(2);
            Row.GeneralAveragePurchasePrice = (Row.TotalPurchaseRealValue == undefined || Row.TotalPurchaseRealValue == 0 || Row.TotalPurchaseQuantity == undefined || Row.TotalPurchaseQuantity == 0 ? 0 : Row.TotalPurchaseRealValue / Row.TotalPurchaseQuantity * 1000).toFixed(2);

        })
        return ReportData;
    }

    $scope.clearEnity = function () {

        //d.getFullYear() + "" + (d.getMonth() + 1) + "" + d.getDate();

        $scope.entryMasterList = [];

        $scope.var.ACC_ID = null;

        $scope.startDate = "";
        $scope.endDate = "";

        $scope.show_print = false;

    }
    
    $scope.fnExcelReport = function () {

    

        //console.log($scope.createArr($scope.entryMasterList));

        //var client_name = $("#cust_Acc_ID option:selected").text();

        //var client_name = $("#cust_Acc_ID").text().split("\n")[1].trim();

        //var arabic = /[\u0600-\u06FF]/;

        var x1 = "بيان المبيعات والمشتريات والكشف والحمل".split(" ").reverse().join(" ");

        //var x2 = ("اسم العميل : " + client_name).split(" ").reverse().join(" ");

        //if (arabic.test(client_name) == false) {

        //    x2 = ("اسم العميل : ").split(" ").reverse().join(" ");

        //    x2 = client_name + " " + x2;
        //}



        pdfMake.fonts = {
            custom: {
                normal: 'DroidKufi-Regular.ttf'
            }
        }

        var docDefinition = {
            content: [
                {
                    //layout: 'lightHorizontalLines', // optional
                    layout: {
                        hLineWidth: function (i, node) {
                            if (i === 0 || i === node.table.body.length) {
                                return 0;
                            }
                            return (i === node.table.headerRows) ? 2 : 1;
                        },
                        vLineWidth: function (i) {
                            return 0;
                        },
                        hLineColor: function (i) {
                            return i === 1 ? 'black' : '#aaa';
                        },
                        paddingLeft: function (i) {
                            return i === 0 ? 0 : 8;
                        },
                        paddingRight: function (i, node) {
                            return (i === node.table.widths.length - 1) ? 0 : 8;
                        },
                        fillColor: function (i, node) {
                            return i % 2 == 0 ? 'white' : '#eee';
                        }
                    },
                    table: {
                        // headers are automatically repeated if the table spans over multiple pages
                        // you can declare how many rows should be treated as headers
                        //widths: [60, 60, 160, 80, 80],

                        headerRows: 2,
                        widths: ['*', '*', 'auto', 'auto', '*', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto'],
                        //margin: [400, 400, 10, 10],
                        body: $scope.createArr($scope.entryMasterList)
                    }
                }
            ],
            images: {
                bee: "data:image/png;base64,"
            },
            pageMargins: [20, 150, 20, 0],
            pageSize: 'A3',
            //pageOrientation: "landscape",
            header: {
                text: x1,// + "\n" + x2,
                alignment: "center",
                margin: [0, 80, 0, 0],
                fontSize: 16,
            },
            background: function (page) {

                return [
                    '\r\n',
                    {
                        image: 'bee',
                        width: 200
                    }
                ];

            },
            defaultStyle: {
                font: 'custom',
                alignment: 'center',
                top: 200
            },
        };
    
        pdfMake.createPdf(docDefinition).download();


    }

    $scope.createArr = function (list) {

        var arr = [];

        for (var i = 0; i <= list.length + 1; i++) {
            arr[i] = [];
        }

        arr[0] = [   
            {
                text: 'النوع'.split(" ").reverse().join(" "),
                alignment: 'center'
            },
            {
                text: "شراء".split(" ").reverse().join(" "),
                colSpan: 6,
                alignment: 'center'
            },
            {},
            {},
            {},
            {},
            {},
            {
                text: "بيع".split(" ").reverse().join(" "),
                colSpan: 6,
                alignment: 'center'
            },
            {},
            {},
            {},
            {},
            {},
            {},
            {},
        ];

        arr[1] = [
            {
                text: 'كشف / حمل',
                alignment: 'center'
            },
            {
                text: 'المتوسط العام للسعر',
                alignment: 'center'
            },
            {
                text: 'اجمالى القيمة بالريال',
                alignment: 'center'
            },
            {
                text: 'اجمالى الكمية المشتراة',
                alignment: 'center'
            },
            {
                text: 'متوسط سعر الكيلوجرام',
                alignment: 'center'
            },
            {
                text: 'القيمة بالريال',
                alignment: 'center'
            },
            {
                text: 'الكمية',
                alignment: 'center'
            },


            {
                text: 'المتوسط العام للسعر',
                alignment: 'center'
            },
            {
                text: 'اجمالى القيمة بالريال',
                alignment: 'center'
            },
            {
                text: 'اجمالى الكمية المشتراة',
                alignment: 'center'
            },
            {
                text: 'متوسط سعر الكيلوجرام',
                alignment: 'center'
            },
            {
                text: 'القيمة بالريال',
                alignment: 'center'
            },
            {
                text: 'الكمية',
                alignment: 'center'
            },

            {
                text: 'التاريخ',
                alignment: 'center'
            },
            {
                text: 'المسلسل',
                alignment: 'center'
            }           
        ];

        for (var i = 0; i < 15; i++) {

            arr[1][i].text = arr[1][i].text.split(" ").reverse().join(" ");
        }

        var index = 2;

        for (var i = 0; i < list.length; i++) {
            arr[index][14] = i + 1;

            var obj = list[i];

            var j = 0;
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (key == "BILL_DATE") {

                        if (obj[key]) {

                            var d1 = new Date(obj[key]);

                            var billDate = d1.getFullYear() + "-" + ("0" + (d1.getMonth() + 1)).slice(-2) + "-" + ("0" + d1.getDate()).slice(-2);

                            arr[index][13] = billDate;
                        }
                        else {
                            arr[index][13] = "";
                        }
                    }
                    else if (key == "SalesActualItemWeightQTY") {

                        arr[index][12] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "SalesAveragepriceperkilogram") {

                        arr[index][11] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "SalesTotalMustPaid") {

                        arr[index][10] = parseFloat(Number(obj[key])).toFixed(2);
                    }

                    else if (key == "TotalSoldQuantity") {

                        arr[index][9] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalSoldRealValue") {

                        arr[index][8] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "GeneralAverageSoldPrice") {

                        arr[index][7] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "PurchasingActualItemWeightQTY") {

                        arr[index][6] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "PurchasingTotalMustPaid") {

                        arr[index][5] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "PurchasingAveragepriceperkilogram") {

                        arr[index][4] = parseFloat(Number(obj[key])).toFixed(2);
                    }

                    else if (key == "TotalPurchaseQuantity") {

                        arr[index][3] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalPurchaseRealValue") {

                        arr[index][2] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "GeneralAveragePurchasePrice") {

                        arr[index][1] = parseFloat(Number(obj[key])).toFixed(2);
                    }
                   
                    else if (key == "DetectionReport") {

                        arr[index][0] = obj[key];
                    }

                    j++;

                }
            }
            index++;
        }
        return arr;
    }
    



}]);


