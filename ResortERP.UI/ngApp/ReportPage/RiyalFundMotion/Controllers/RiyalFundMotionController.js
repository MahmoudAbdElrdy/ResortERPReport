...
app.controller('RiyalFundMotionController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService','compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, compBranchesService) {
    ...
    $scope.getAccountList = function () {
        accountsService.GetAccountsFilteredByType("Fund").then(function (result) {
    ...
    $scope.GetSearchResult = function () {
        if ($scope.var.ACC_ID && $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            ...
            var ReportName = "getAccountStatementDailyReport";
            //ASPX page URL to load report  
            var src = '../Reports/ReportForm/ReportPage.aspx?';
            //We can add parameters here  
            src = src + "ReportName=" + ReportName + "&companyBranches=" + companyBranches + "&Accounts=" + $scope.var.ACC_ID + "&startDate=" + startDate + "&endDate=" + endDate + "&arStartDate=" + arStartDate + "&arEndDate=" + arEndDate + "&ReportType=" + $scope.ReportType + "&CompanyName=" + CompanyName;
    ...
    $scope.fnExcelReport = function () {
        ...
        var x1 = "حركة صندوق الريال (سندات )".split(" ").reverse().join(" ");
