'use strict';
app.controller('AccountBalancesController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService', 'costCentersService', 'ReportsService', 'compBranchesService', 'entrySettingService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, costCentersService, ReportsService, compBranchesService, entrySettingService) {

    $scope.accountList = [];

    $scope.entryMasterList = [];

    $scope.cust_Acc_ID = null;

    $scope.startDate = "";
    $scope.endDate = "";

    $scope.show_print = false;
    $scope.var = { ACC_ID: null, CURRENCY_ID: null, COST_CENTER_ID: null };

    /******************************/

    $scope.mainCostCenterList = [];

    $scope.currenciesList = [];

    $scope.MainAcc = true;
    $scope.SubAcc = true;
    $scope.BalancedAcc = true;
    $scope.EmptyAcc = true;

    $scope.CURRENCY_RATE = null;

    $scope.type = 0;


    $scope.selectedCostCenter = null;
    $scope.selectedCurrency = null;

    $scope.sum_DEBIT = [];
    $scope.sum_CREDIT = [];
    $scope.sum_Balance = [];


    $scope.cust_Acc_ID_selected = [];

    $scope.index = 0;

    $scope.count = 0;

    $scope.currentList = null;

    $scope.UserBranches = [];
    $scope.dropdownSetting = {
        scrollable: true,
        scrollableHeight: '300px',
        checkBoxes: true,
        enableSearch: true,
        buttonClasses: 'btn btn-default btn-block',
        styleActive: true,
        smartButtonTextProvider: function (selectedItems) {
            if (selectedItems.length > 3) {
                return "تم اختيار " + selectedItems.length + ' فروع';
            }
            else if (selectedItems.length == 3) {
                return selectedItems[0].label + ', ' + selectedItems[1].label + ', ' + selectedItems[2].label;
            }
            else if (selectedItems.length == 2) {
                return selectedItems[0].label + ', ' + selectedItems[1].label;
            }
            else if (selectedItems.length == 1) {
                return selectedItems[0].label;
            }
            else {
                return 'اختار فرع';
            }
        }
    }

    /******************************/

    $scope.GetEntrySettings = function () {
        entrySettingService.getAll().then(function (response) {
            $scope.entrySettingsList = response.data;
        }, function (error) { console.log(error.data.message); });
    };

    $scope.getAccountList = function () {       
        var type = $location.search().ReportType == 1 ||
            $location.search().ReportType == 2 ||
            $location.search().ReportType == 3
            ? "C"
            : $location.search().ReportType == 5
            ? "S"
            : $location.search().ReportType == 4
            ? "A"
            : "";
        ReportsService.getAccountsByType(type).then(function (results) {
            $scope.accountList = results.data;
            $('#cust_Acc_ID').multiSelect({
                selectableHeader: "<input type='text' class='search-input form-control' autocomplete='off' placeholder=''>",
                selectionHeader: "<input type='text' class='search-input form-control' autocomplete='off' placeholder=''>",
                afterInit: function (ms) {
                    var that = this,
                        $selectableSearch = that.$selectableUl.prev(),
                        $selectionSearch = that.$selectionUl.prev(),
                        selectableSearchString = '#' + that.$container.attr('id') + ' .ms-elem-selectable:not(.ms-selected)',
                        selectionSearchString = '#' + that.$container.attr('id') + ' .ms-elem-selection.ms-selected';

                    that.qs1 = $selectableSearch.quicksearch(selectableSearchString)
                        .on('keydown', function (e) {
                            if (e.which === 40) {
                                that.$selectableUl.focus();
                                return false;
                            }
                        });

                    that.qs2 = $selectionSearch.quicksearch(selectionSearchString)
                        .on('keydown', function (e) {
                            if (e.which == 40) {
                                that.$selectionUl.focus();
                                return false;
                            }
                        });
                },
                afterSelect: function () {
                    this.qs1.cache();
                    this.qs2.cache();
                },
                afterDeselect: function () {
                    this.qs1.cache();
                    this.qs2.cache();
                }
            });

            $.each(results.data, function (index, value) {
                $('#cust_Acc_ID').multiSelect('addOption', { value: value.OptionValue, text: value.OptionText, index: index });
            });

        }, function (error) {
            console.log(error.data.message);
        });   
    }


    $scope.getAllCurrencies = function () {

        currencyService.getAll().then(function (response) {

            var result = response.data;
            $scope.currenciesList = result;
            //console.log(result);
        })
    }

    $scope.AccountBalancesPageLoad = function () {
        if ($location.search().ReportType == undefined || $location.search().ReportType == null) {
            $location.path('/home');
        } else {
            switch ($location.search().ReportType) {
                case "1":
                    $scope.ReportTitle = "العملاء الراكدة";
                    $scope.ReportType = 1;
                    break;
                case "2":
                    $scope.ReportTitle = "العملاء الأكثر حركة";
                    $scope.ReportType = 2;
                    break;
                case "3":
                    $scope.ReportTitle = "أرصدة العملاء";
                    $scope.ReportType = 3;
                    break;
                case "4":
                    $scope.ReportTitle = "ارصدة الحسابات";
                    $scope.ReportType = 4;
                    break;
                case "5":
                    $scope.ReportTitle = "ارصدة الموردين";
                    $scope.ReportType = 3;
                    break;
                default:
                    {
                        $location.path('/home');
                    }
            }
        }
        //$scope.loadMultiSelect();

        $scope.getAccountList();

        $scope.getCostCentersList();
        $scope.getAllCurrencies();
        $scope.GetUserBranches();
        $scope.GetEntrySettings();
        $scope.accountBalance = {Number:10};
        $scope.startDate = new Date(2018, 8, 14);
        $scope.endDate = new Date();
    }

    $scope.GetUserBranches = function () {
        compBranchesService.getUserBranches().then(function (response) {
            if (response.data != undefined && response.data.length > 0) {
                angular.forEach(response.data, function (value, index) {
                    $scope.UserBranches.push({ id: value.COM_BRN_ID, label: value.COM_BRN_AR_NAME });
                });
                $scope.SelectedBranch = [$scope.UserBranches[0]];
            }
        }, function (error) { console.log(error.data.message); });
    };

    $scope.getCostCentersList = function () {
        costCentersService.getMainCostCenters().then(function (results) {
            $scope.mainCostCenterList = results.data;
            //console.log(results.data);
        }, function (error) {
            console.log(error.data.message);
        });
    }

    $scope.changeCurrencyRate = function (selectedItem) {

        //alert($scope.var.CURRENCY_ID);

        $scope.selectedCurrency =selectedItem;

        $scope.CURRENCY_RATE = selectedItem.CURRENCY_RATE;
    };

    $scope.getCostCenter = function (selectedItem) {
        $scope.selectedCostCenter = selectedItem;
    }

    $scope.GetSearchResult = function () {
        //$scope.var.ACC_ID &&
        $scope.accountBalance.ReportType = $scope.ReportType;

        var cust_Acc_ID = $('#cust_Acc_ID').val();
        var entrySettingList =
            $filter('filter')($scope.entrySettingsList, function (item) { return item.selected === true });

        if (cust_Acc_ID && $scope.startDate && $scope.endDate && entrySettingList.length > 0) {
            $scope.accountBalance.ReportSources = [];
            angular.forEach(entrySettingList,
                function(item, index) {
                    $scope.accountBalance.ReportSources.push(item.ENTRY_SETTING_ID);
                });

            var Accounts_Xml = "";
            for (var i = 0; i < cust_Acc_ID.length; i++) {
                Accounts_Xml += "<ACCOUNTS><ACC_ID>" + cust_Acc_ID[i] + "</ACC_ID></ACCOUNTS>";
            }
            var Accounts = "<DS>" + Accounts_Xml + "</DS>";
            $scope.accountBalance.Accounts = Accounts;

            var d1 = new Date($scope.startDate);

            $scope.accountBalance.startDate = $scope.startDate; //d1.getFullYear() + "" + ("0" + (d1.getMonth() + 1)).slice(-2) + "" + ("0" + d1.getDate()).slice(-2);

            var d2 = new Date($scope.endDate);

            $scope.accountBalance.endDate = $scope.endDate;// d2.getFullYear() + "" + ("0" + (d2.getMonth() + 1)).slice(-2) + "" + ("0" + (d2.getDate())).slice(-2);

            ReportsService.getAccountBalancesReportResult($scope.accountBalance).then(function (results) {
                $scope.ReportResult = results.data;
                $scope.show_print = true;
            });
        }
        else swal("عفوا", "يجب اختيار الحسابات ومصادر التقارير والتواريخ ", "error");
    }

    $scope.clearEnity = function () {

        $scope.entryMasterList = [];
        $scope.var.ACC_ID = null;
        $scope.startDate = new Date(2018, 8, 14);
        $scope.endDate = new Date();

        $scope.show_print = false;

        $scope.currentList = null;
        $scope.accountBalance = { Number: 10};
    }

    $scope.loadMultiSelect = function () {
        $(document).ready(function () {
            //$('#cust_Acc_ID').multiSelect('refresh');  
            //console.log($('#cust_Acc_ID').multiSelect());
            $('#cust_Acc_ID').multiSelect({
                selectableHeader: "<input type='text' class='search-input form-control' autocomplete='off' placeholder=''>",
                selectionHeader: "<input type='text' class='search-input form-control' autocomplete='off' placeholder=''>",
                afterInit: function (ms) {
                    var that = this,
                        $selectableSearch = that.$selectableUl.prev(),
                        $selectionSearch = that.$selectionUl.prev(),
                        selectableSearchString = '#' + that.$container.attr('id') + ' .ms-elem-selectable:not(.ms-selected)',
                        selectionSearchString = '#' + that.$container.attr('id') + ' .ms-elem-selection.ms-selected';

                    that.qs1 = $selectableSearch.quicksearch(selectableSearchString)
                        .on('keydown', function (e) {
                            if (e.which === 40) {
                                that.$selectableUl.focus();
                                return false;
                            }
                        });

                    that.qs2 = $selectionSearch.quicksearch(selectionSearchString)
                        .on('keydown', function (e) {
                            if (e.which == 40) {
                                that.$selectionUl.focus();
                                return false;
                            }
                        });
                },
                afterSelect: function () {
                    this.qs1.cache();
                    this.qs2.cache();
                },
                afterDeselect: function () {
                    this.qs1.cache();
                    this.qs2.cache();
                }
            });
            console.log($('#cust_Acc_ID').multiSelect());

        });
    }
   
    $scope.getTableWidthSpan = function () {
        var tableSpan = 6;
        if ($scope.PreviousBalance) {
            tableSpan += 3;
        }
        if ($scope.LastCreditTransaction) {
            tableSpan += 1;
        }
        if ($scope.LastDebitTransaction) {
            tableSpan += 1;
        }
        return tableSpan;
    };

    $scope.getTableWidths = function (size) {
        var arrayWidths = [];
        for (var i = 0; i < size; i++) {
            arrayWidths.push("*");
        }
        return arrayWidths;
    }

    $scope.PrintReport = function () {
        pdfMake.fonts = {
            custom: {
                normal: 'DroidKufi-Regular.ttf'
            }
        }

        var tableLayout = {
            hLineWidth: function (i, node) {
                if (i === 0 || i === node.table.body.length) {
                    return 0;
                }
                return (i === node.table.headerRows) ? 2 : 1;
            },
            vLineWidth: function (i) {
                return 0;
            },
            hLineColor: function (i) {
                return i === 1 ? 'black' : '#aaa';
            },
            paddingLeft: function (i) {
                return i === 0 ? 0 : 8;
            },
            paddingRight: function (i, node) {
                return (i === node.table.widths.length - 1) ? 0 : 8;
            },
            fillColor: function (i, node) {
                return i % 2 == 0 ? 'white' : '#eee';
            }
        };

        var docDefinition = {
            content: [
                {
                    table: {
                        headerRows: 1,
                        widths: $scope.getTableWidths($scope.getTableWidthSpan()),
                        body: $scope.CreateReportTableBody()
                    },
                    layout: tableLayout
                }
            ],
            styles: {
                subheader: {
                    fontSize: 16,
                }
            },
            images: {
                bee: "data:image/png;base64,"
            },
            pageMargins: [20, 150, 20, 0],
            pageOrientation: "portrait",
            pageSize: 'A3',
            header: {
                text: (" تقرير " + $scope.ReportTitle).split(" ").reverse().join(" "),
                alignment: "center",
                margin: [0, 80, 0, 0],
                fontSize: 16,
            },
            background: function (page) {

                return [
                    '\r\n',
                    {
                        image: 'bee',
                        width: 200
                    }
                ];

            },
            defaultStyle: {
                font: 'custom',
                alignment: 'center',
                top: 200
            },
        };

        pdfMake.createPdf(docDefinition).download();

    }

    $scope.CreateReportTableBody = function () {
        var tableData = [];
        tableData[0] = [];
        if ($scope.ReportType == 1 || $scope.ReportType == 2) {
            tableData[0].push({
                text: "عدد الحركات".split(" ").reverse().join(" "),
            });
        }
        if ($scope.LastDebitTransaction) {
            tableData[0].push({ text: "اخر حركة مدينة".split(" ").reverse().join(" ") });
        }
        if ($scope.LastCreditTransaction) {
            tableData[0].push({ text: "اخر حركة دائنة".split(" ").reverse().join(" ") });
        }
        tableData[0].push(
            {
                text: "الرصيد",
            },
            {
                text: "مدين",
            },
            {
                text: "دائن",
            }
        );
        if ($scope.PreviousBalance) {
            tableData[0].push(
                { text: "الرصيد السابق".split(" ").reverse().join(" ") },
                { text: "مدين سابق".split(" ").reverse().join(" ") },
                { text: "دائن سابق".split(" ").reverse().join(" ") },
            );
        }
        tableData[0].push(
            {
                text: "الحساب",
            },
            {
                text: "مسلسل",
            }            
        );
        for (var i = 0; i < $scope.ReportResult.Transactions.length; i++) {
            tableData[i + 1] = [];
            if ($scope.ReportType == 1 || $scope.ReportType == 2) {
                tableData[i + 1].push({
                    text: $scope.ReportResult.Transactions[i].NUMBER != undefined &&
                        $scope.ReportResult.Transactions[i].NUMBER != null
                        ? $scope.ReportResult.Transactions[i].NUMBER
                        : 0,
                });
            }
            if ($scope.LastDebitTransaction) {
                tableData[i + 1].push({
                    text: $scope.ReportResult.Transactions[i].LAST_DEBIT != undefined && $scope.ReportResult.Transactions[i].LAST_DEBIT != null
                        ? $scope.ReportResult.Transactions[i].LAST_DEBIT : "",
                });
            }
            if ($scope.LastCreditTransaction) {
                tableData[i + 1].push({
                    text: $scope.ReportResult.Transactions[i].LAST_CREDIT != undefined && $scope.ReportResult.Transactions[i].LAST_CREDIT != null
                        ? $scope.ReportResult.Transactions[i].LAST_CREDIT
                        : "",
                });
            }
            tableData[i + 1].push(
                {
                    text: $scope.ReportResult.Transactions[i].Balance != undefined &&
                        $scope.ReportResult.Transactions[i].Balance != null
                        ? $scope.ReportResult.Transactions[i].Balance.toFixed(2)
                        : 0,
                },
                {
                    text: $scope.ReportResult.Transactions[i].DEBIT != undefined &&
                        $scope.ReportResult.Transactions[i].DEBIT != null
                        ? $scope.ReportResult.Transactions[i].DEBIT.toFixed(2)
                        : 0,
                },
                {
                    text: $scope.ReportResult.Transactions[i].CREDIT != undefined &&
                        $scope.ReportResult.Transactions[i].CREDIT != null
                        ? $scope.ReportResult.Transactions[i].CREDIT.toFixed(2)
                        : 0,
                }
            );
            if ($scope.PreviousBalance) {
                tableData[i + 1].push(
                    {
                        text: $scope.ReportResult.Transactions[i].PreviousBalance != undefined &&
                            $scope.ReportResult.Transactions[i].PreviousBalance != null
                            ? $scope.ReportResult.Transactions[i].PreviousBalance.toFixed(2)
                            : "0",
                    },
                    {
                        text: $scope.ReportResult.Transactions[i].PreviousDebit != undefined &&
                            $scope.ReportResult.Transactions[i].PreviousDebit != null
                            ? $scope.ReportResult.Transactions[i].PreviousDebit.toFixed(2)
                            : "0",
                    },
                    {
                        text: $scope.ReportResult.Transactions[i].PreviousCredit != undefined &&
                            $scope.ReportResult.Transactions[i].PreviousCredit != null
                            ? $scope.ReportResult.Transactions[i].PreviousCredit.toFixed(2)
                            : "0",
                    },
                );
            }
            tableData[i + 1].push(
                {
                    text: $scope.ReportResult.Transactions[i].ACC_AR_NAME != undefined &&
                        $scope.ReportResult.Transactions[i].ACC_AR_NAME != null
                        ? $scope.ReportResult.Transactions[i].ACC_AR_NAME.split(" ").reverse().join(" ")
                        : "",
                },
                {
                    text: i + 1
                }
            );
        }
        if ($scope.TotalBalances) {
            var length = tableData.length;
            tableData[length] = [];
            if ($scope.ReportType == 1 || $scope.ReportType == 2 ) {
                tableData[length].push({});
            }
            if ($scope.LastDebitTransaction) {
                tableData[length].push({});
            }
            if ($scope.LastCreditTransaction) {
                tableData[length].push({});
            }
            tableData[length].push(
                {
                    text: $scope.ReportResult.TotalBalance.toFixed(2),
                },
                {
                    text: $scope.ReportResult.TotalDebit.toFixed(2),
                },
                {
                    text: $scope.ReportResult.TotalCredit.toFixed(2),
                }
            );
            if ($scope.PreviousBalance) {
                tableData[length].push(
                    {
                        text: $scope.ReportResult.TotalPrevBalance.toFixed(2),
                    },
                    {
                        text: $scope.ReportResult.TotalPrevDebit.toFixed(2),
                    },
                    {
                        text: $scope.ReportResult.TotalPrevCredit.toFixed(2),
                    },
                );
            }
            tableData[length].push(
                {
                    text: "المجموع",
                    colSpan:2
                }
            )
        }
        return tableData;
    }


}]);


