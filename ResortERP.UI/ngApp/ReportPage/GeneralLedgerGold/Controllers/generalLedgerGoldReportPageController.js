'use strict';
app.controller('generalLedgerGoldController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService', 'costCentersService', 'generalLedgerGoldService','compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, costCentersService, generalLedgerGoldService, compBranchesService) {
    ...
    $scope.sum_DEBIT_24 = [];
    $scope.sum_CREDIT_24 = [];
    $scope.sum_Balance_24 = [];

    $scope.sum_DEBIT_22 = [];
    $scope.sum_CREDIT_22 = [];
    $scope.sum_Balance_22 = [];

    $scope.sum_DEBIT_21 = [];
    $scope.sum_CREDIT_21 = [];
    $scope.sum_Balance_21 = [];

    $scope.sum_DEBIT_18 = [];
    $scope.sum_CREDIT_18 = [];
    $scope.sum_Balance_18 = [];
    ...
    $scope.generalLedgerGoldPageLoad = function () {
    ...
    $scope.GetSearchResult = function () {
        ...
        if (cust_Acc_ID && $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            ...
            var costCenterId = $scope.var.COST_CENTER_ID;

            generalLedgerGoldService.getByAccounts(companyBranches, Accounts, costCenterId, source,startDate, endDate).then(function (results) {
                ...
                for (var i = 0; i < results.data.length; i++) {
                    ...
                    var sum_DEBIT_24 = 0;
                    var sum_CREDIT_24 = 0;
                    var sum_Balance_24 = 0;

                    var sum_DEBIT_22 = 0;
                    var sum_CREDIT_22 = 0;
                    var sum_Balance_22 = 0;

                    var sum_DEBIT_21 = 0;
                    var sum_CREDIT_21 = 0;
                    var sum_Balance_21 = 0;

                    var sum_DEBIT_18 = 0;
                    var sum_CREDIT_18 = 0;
                    var sum_Balance_18 = 0;
                    ...
                    $.each(list, function (index, value) {
                        ...
                        // gold
                        sum_DEBIT_24 += value.DEBIT_Gold24;
                        sum_CREDIT_24 += value.CREDIT_Gold24;
                        sum_Balance_24 += value.BAL_Gold24;

                        sum_DEBIT_22 += value.DEBIT_Gold22;
                        sum_CREDIT_22 += value.CREDIT_Gold22;
                        sum_Balance_22 += value.BAL_Gold22;

                        sum_DEBIT_21 += value.DEBIT_Gold21;
                        sum_CREDIT_21 += value.CREDIT_Gold21;
                        sum_Balance_21 += value.BAL_Gold21;

                        sum_DEBIT_18 += value.DEBIT_Gold18;
                        sum_CREDIT_18 += value.CREDIT_Gold18;
                        sum_Balance_18 += value.BAL_Gold18;
                    });
                    ...
                    //gold
                    $scope.sum_DEBIT_24.push(sum_DEBIT_24);
                    $scope.sum_CREDIT_24.push(sum_CREDIT_24);
                    $scope.sum_Balance_24.push(sum_Balance_24);

                    $scope.sum_DEBIT_22.push(sum_DEBIT_22);
                    $scope.sum_CREDIT_22.push(sum_CREDIT_22);
                    $scope.sum_Balance_22.push(sum_Balance_21);

                    $scope.sum_DEBIT_21.push(sum_DEBIT_21);
                    $scope.sum_CREDIT_21.push(sum_CREDIT_21);
                    $scope.sum_Balance_21.push(sum_Balance_21);

                    $scope.sum_DEBIT_18.push(sum_DEBIT_18);
                    $scope.sum_CREDIT_18.push(sum_CREDIT_18);
                    $scope.sum_Balance_18.push(sum_Balance_18);
                }
                console.log("--------------");
            ...
            content: [
                {
                    ...
                    table: {
                        ...
                        headerRows: 1,
                        widths: [55, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55, 130, 30, 30, 70],
                        margin: [400, 400, 0, 0],
    ...
    $scope.createArr = function (list) {
        ...
        arr[0][12] = "نوع المستند";
        arr[0][13] = "رقم المستند";
        arr[0][14] = "رقم القيد";
        arr[0][15] = "التاريخ";
            ...
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    ...
                    if (key == "ENTRY_NUMBER") {
                        ...
                        if (i == 0) {
                            arr[index][12] = "الرصيد السابق".split(" ").reverse().join(" ");
                        }
                        else {
                            if (obj[key]) {
                                arr[index][12] = obj[key].split(" ").reverse().join(" ");
                            }
                            else {
                                arr[index][12] = "";
                            }
                        ...
                        if (obj[key]) {
                            ...
                            arr[index][15] = account_date
                        }
                        else {
                            arr[index][15] = "";
                        }
                    }
                    else if (key == "BAL_Gold24") {
        ...
        var header = [
            "24 صافى الرصيد", "مجموع مدين 24", "24 مجموع دائن",
            "22 صافى الرصيد", "مجموع مدين 22", "22 مجموع دائن",
            "21 صافى الرصيد", "مجموع مدين 21", "21 مجموع دائن",
            "18 صافى الرصيد", "مجموع مدين 18", "18 مجموع دائن",
            "", "", "", ""];

        var total = [
            $scope.sum_Balance_24[_index], $scope.sum_DEBIT_24[_index], $scope.sum_CREDIT_24[_index],
            $scope.sum_Balance_22[_index], $scope.sum_DEBIT_22[_index], $scope.sum_CREDIT_22[_index],
            $scope.sum_Balance_21[_index], $scope.sum_DEBIT_21[_index], $scope.sum_CREDIT_21[_index],
            $scope.sum_Balance_18[_index], $scope.sum_DEBIT_18[_index], $scope.sum_CREDIT_18[_index],
            null, null, null, null];
