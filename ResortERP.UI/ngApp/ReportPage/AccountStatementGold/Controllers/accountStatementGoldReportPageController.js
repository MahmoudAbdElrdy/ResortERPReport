'use strict';
app.controller('accountStatementGoldController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService', 'compBranchesService', 'sharedService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, compBranchesService, sharedService) {
    ...
    $scope.getAccountList = function () {
        accountsService.getAll().then(function (results) {
            $scope.accountList = results.data.filter(x => x.ACC_TYPE_ID == 13 && x.ACC_STATE==2);
    ...
    $scope.GetSearchResult = function () {
        ...
        if ($scope.var.ACC_ID && $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            ...
            var companyBranches = '';
            var CompanyName = '';
            angular.forEach($scope.SelectedBranch,
                function(item, index) {
            ...
            var ReportName = "Get_PRC_RPT_LEDGER_Gold";
            //ASPX page URL to load report  
            var src = '../Reports/ReportForm/ReportPage.aspx?';
            //We can add parameters here  
            src = src + "ReportName=" + ReportName + "&companyBranches=" + companyBranches + "&Accounts=" + $scope.var.ACC_ID + "&startDate=" + startDate + "&endDate=" + endDate + "&arStartDate=" + arStartDate + "&arEndDate=" + arEndDate + "&CompanyName=" + CompanyName;
            ...
            //accountStatementService.getByAccountsGold(companyBranches, Accounts, startDate, endDate).then(function (results) {
    ...
    $scope.ConvertDate = function (DATE) {
        if (DATE != null) {
            ...
            var days = ["اﻷحد", "اﻷثنين", "الثلاثاء", "اﻷربعاء", "الخميس", "الجمعة", "السبت"];
            var delDateString = DATE.getFullYear() + ' ' + months[DATE.getMonth()] + '   ' + DATE.getDate() + '  ' + days[DATE.getDay()];
    ...
    $scope.fnExcelReport = function () {
        ...
        //var x1 = "كشف حساب عميل ذهب".split(" ").reverse().join(" ");
        var x1 = "كشف حساب ".split(" ").reverse().join(" ");
        ...
        var FristDate = $scope.ConvertDate(dd1);
        var LastDate = $scope.ConvertDate(dd2);
    ...
    $scope.createArr = function (list) {
        ...
        var FristHeader = [
            ...
            '', '',
            { text: 'البيان', style: 'tableHeader: {bold: true, fontSize: 13,color}', colSpan: 6, alignment: 'center' }
            , '', '', '', '', '',
