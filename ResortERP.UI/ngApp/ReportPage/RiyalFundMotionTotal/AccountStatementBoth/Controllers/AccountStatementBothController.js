'use strict';
app.controller('accountStatementBothController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementDailyService','compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementDailyService, compBranchesService) {
    ...
    $scope.getAccountList = function () {
        accountsService.getAll().then(function (results) {
            $scope.accountList = results.data.filter(x => x.ACC_STATE == 2 && (x.ACC_TYPE_ID == 1 || x.ACC_TYPE_ID == 13 || x.ACC_TYPE_ID == 8 || x.ACC_TYPE_ID == 9 || x.ACC_TYPE_ID == 10));
    ...
    $scope.GetSearchResult = function () {
        if ($scope.var.ACC_ID && $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            ...
            var ReportName = "getAccountStatementGoldClientReport";
            //ASPX page URL to load report  
            var src = '../Reports/ReportForm/ReportPage.aspx?';
            //We can add parameters here  
            src = src + "ReportName=" + ReportName + "&companyBranches=" + companyBranches + "&Accounts=" + $scope.var.ACC_ID + "&startDate=" + startDate + "&endDate=" + endDate + "&arStartDate=" + arStartDate + "&arEndDate=" + arEndDate + "&CompanyName=" + CompanyName + "&AccountName=" + AccountName;
    ...
    $scope.fnExcelReport = function () {
        ...
        var x1 = "كشف حساب عميل يومى".split(" ").reverse().join(" ");
