...
app.controller('trialBalanceController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService', 'costCentersService', 'trialBalanceService', 'compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, costCentersService, trialBalanceService, compBranchesService) {
    ...
    $scope.GetSearchResult = function () {
        ...
        if ( $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) { 
            ...
            trialBalanceService.getByAccounts(companyBranches, startDate, endDate, 1).then(function (results) {
                ...
                $.each($scope.accountList, function (index, value) {
                    $scope.sum_BDEBIT += value.BDEBIT ? parseFloat(value.BDEBIT) : 0;
                    $scope.sum_BCREDIT += value.BCREDIT ? parseFloat(value.BCREDIT) : 0;
                    $scope.sum_INDEBIT += value.INDEBIT ? parseFloat(value.INDEBIT) : 0;
                    $scope.sum_INCREDIT += value.INCREDIT ? parseFloat(value.INCREDIT) : 0;
                    $scope.sum_ADEBIT += value.ADEBIT ? parseFloat(value.ADEBIT) : 0;
                    $scope.sum_ACREDIT += value.ACREDIT ? parseFloat(value.ACREDIT) : 0;
                })
            ...
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (key == "ACREDIT") {
                        ...
                        arr[index][6] = obj[key] ? obj[key].split(" ").reverse().join(" ") : '';
                    }
                    else if (key == "AC_CODE") {
                        arr[index][7] = obj[key] ? obj[key].split(" ").reverse().join(" ") : '';
                    }
                }
            }
            index++;
