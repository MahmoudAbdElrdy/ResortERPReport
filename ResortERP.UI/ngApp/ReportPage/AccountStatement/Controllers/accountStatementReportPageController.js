'use strict';
app.controller('accountStatementController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService','compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, compBranchesService) {
    ...
    $scope.getAccountList = function () {
        accountsService.getAll().then(function (results) {
            $scope.accountList = results.data.filter(x => (x.ACC_STATE == 2) && (x.ACC_TYPE_ID == 5 || x.ACC_TYPE_ID == 7));
    ...
    $scope.accountStatmentPageLoad = function () {
    ...
    $scope.GetNameofAccount = function (accId) {
        
        var AccountData = $scope.accountList.find(x => x.ACC_ID == accId);
        return AccountData.ACC_AR_NAME;
    };
    ...
    $scope.GetSearchResult = function () {
        if ($scope.var.ACC_ID && $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            var companyBranches = '';
            var CompanyName = '';
            var AccountName = $scope.GetNameofAccount($scope.var.ACC_ID);
            ...
            if ($scope.startDate != null)
                arStartDate = $scope.ConvertAllDate($scope.startDate);
            else
                arStartDate = null;
            //Name Function in aspx
            var ReportName = "getAccountStatementReport";
            //ASPX page URL to load report  
            var src = '../Reports/ReportForm/ReportPage.aspx?';
            //We can add parameters here  
            src = src + "ReportName=" + ReportName + "&companyBranches=" + companyBranches + "&Accounts=" + $scope.var.ACC_ID + "&startDate=" + startDate + "&endDate=" + endDate + "&arStartDate=" + arStartDate + "&arEndDate=" + arEndDate + "&ReportType=" + $scope.ReportType + "&CompanyName=" + CompanyName + "&AccountName=" + AccountName;
            ...
            //accountStatementService.getByAccounts(companyBranches, Accounts, startDate, endDate).then(function (results) {
    ...
    $scope.fnExcelReport = function () {
        ...
        var x1 = "كشف حساب عميل".split(" ").reverse().join(" ");
            ...
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (key == "BAL") {
                        ...
                        if (obj[key]) {
                            arr[index][4] = obj[key].split(" ").reverse().join(" ");
                        }
                        else {
                            arr[index][4] = '';
                        }
                        ...
                        if (obj[key]) {
                            arr[index][6] = obj[key].split(" ").reverse().join(" ");
                        }
                        else if (obj["CHEET"]) {
                            arr[index][6] = obj["CHEET"].split(" ").reverse().join(" ");
                        }
                        else {
                            arr[index][6] = obj["NOTE"].split(" ").reverse().join(" ");
                        }
                    }
                    else if (key == "DATE") {
                        if (obj[key]) {
