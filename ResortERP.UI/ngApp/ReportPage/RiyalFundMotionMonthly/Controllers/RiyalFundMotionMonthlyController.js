'use strict';
app.controller('RiyalFundMotionMonthlyController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementMonthlyService','compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementMonthlyService, compBranchesService) {
    ...
    $scope.GetSearchResult = function () {
        ...
        if ($scope.var.ACC_ID && $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            ...
            accountStatementMonthlyService.getReportResult(companyBranches, Accounts, startDate, endDate).then(function (results) {
    ...
    $scope.fnExcelReport = function () {
        ...
        var x1 = "حركة صندوق الريال (شهرية )".split(" ").reverse().join(" ");
                ...
                if (obj.hasOwnProperty(key)) {
                    if (key == "BAL") {
                        ...
                        if (obj[key]) {
                            ...
                            arr[index][7] = obj[key];// account_date
                        }
                        else {
                            arr[index][7] = "";
                        }
