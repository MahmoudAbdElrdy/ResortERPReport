'use strict';
app.controller('SalesTaxReportController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementService', 'compBranchesService', 'costCentersService', 'SalesTaxReportService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementService, compBranchesService, costCentersService, SalesTaxReportService) {

    $scope.ReportData = [];
    $scope.UserBranches = [];
    $scope.dropdownSetting = {
        scrollable: true,
        scrollableHeight: '300px',
        checkBoxes: true,
        enableSearch: true,
        buttonClasses: 'btn btn-default btn-block',
        styleActive: true,
        smartButtonTextProvider: function (selectedItems) {
            if (selectedItems.length > 3) {
                return "تم اختيار " + selectedItems.length + ' فروع';
            }
            else if (selectedItems.length == 3) {
                return selectedItems[0].label + ', ' + selectedItems[1].label + ', ' + selectedItems[2].label;
            }
            else if (selectedItems.length == 2) {
                return selectedItems[0].label + ', ' + selectedItems[1].label;
            }
            else if (selectedItems.length == 1) {
                return selectedItems[0].label;
            }
            else {
                return 'اختار فرع';
            }
        }
    }

    $scope.startDate = "";
    $scope.endDate = "";
    $scope.show_print = false;

    $scope.SalesTaxReportPageLoad = function () {
        $scope.GetUserBranches();
        $scope.startDate = new Date(2018, 8, 14);
        $scope.endDate = new Date();
    }

    $scope.GetUserBranches = function () {
        compBranchesService.getUserBranches().then(function (response) {
            if (response.data != undefined && response.data.length > 0) {
                angular.forEach(response.data, function (value, index) {
                    $scope.UserBranches.push({ id: value.COM_BRN_ID, label: value.COM_BRN_AR_NAME });
                });
                $scope.SelectedBranch = [$scope.UserBranches[0]];
            }
        }, function (error) { console.log(error.data.message); });
    };

    $scope.GetSearchResult = function () {
        if ($scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {
            var companyBranches = '';
            angular.forEach($scope.SelectedBranch, function (item, index) {
                companyBranches += item.id + ',';
            })
            companyBranches = companyBranches.substr(0, companyBranches.length - 1);
            var d1 = new Date($scope.startDate);
            var startDate = d1.getFullYear() + "" + ("0" + (d1.getMonth() + 1)).slice(-2) + "" + ("0" + d1.getDate()).slice(-2);
            var d2 = new Date($scope.endDate);
            var endDate = d2.getFullYear() + "" + ("0" + (d2.getMonth() + 1)).slice(-2) + "" + ("0" + (d2.getDate())).slice(-2);

            SalesTaxReportService.getReportData(companyBranches, startDate, endDate).then(function (results) {
                if (results != undefined && results.data != undefined && results.data.length > 0) {
                    $scope.ReportData = results.data;
                    $scope.show_print = true;
                }
                else {
                    $scope.ReportData = [];
                }
            });
        }
        else swal("عفوا", " لابد من البحث بالتاريخ والفرع", "error");
    }

    $scope.clearEnity = function () {
        $scope.ReportData = [];
        $scope.startDate = new Date(2018, 8, 14);
        $scope.endDate = new Date();
        $scope.show_print = false;
    }
    
    $scope.PdfPrintReport = function () {

        var x1 = "تقرير ضريبة المبيعات".split(" ").reverse().join(" ");
        pdfMake.fonts = {
            custom: {
                normal: 'DroidKufi-Regular.ttf'
            }
        }

        var docDefinition = {
            content: [
                {
                    //layout: 'lightHorizontalLines', // optional
                    layout: {
                        hLineWidth: function (i, node) {
                            if (i === 0 || i === node.table.body.length) {
                                return 0;
                            }
                            return (i === node.table.headerRows) ? 2 : 1;
                        },
                        vLineWidth: function (i) {
                            return 0;
                        },
                        hLineColor: function (i) {
                            return i === 1 ? 'black' : '#aaa';
                        },
                        paddingLeft: function (i) {
                            return i === 0 ? 0 : 8;
                        },
                        paddingRight: function (i, node) {
                            return (i === node.table.widths.length - 1) ? 0 : 8;
                        },
                        fillColor: function (i, node) {
                            return i % 2 == 0 ? 'white' : '#eee';
                        }
                    },
                    table: {
                        // headers are automatically repeated if the table spans over multiple pages
                        // you can declare how many rows should be treated as headers
                        //widths: [60, 60, 160, 80, 80],

                        headerRows: 1,
                        widths: [75, 45, 45, 60, 'auto', 'auto', 'auto', 'auto', 45, 25, 55, 30, 30, 30, 30, 25, 45, 35, 40, 35, 'auto', 'auto'],

                        //widths: ['*', '*', 'auto', 'auto', '*', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto'],
                        //margin: [400, 400, 10, 10],
                        body: $scope.createArr($scope.ReportData)
                    }
                }
            ],
            images: {
                bee: "data:image/png;base64,"
            },
            pageMargins: [5, 150, 5, 0],
            pageSize: 'A3',
            pageOrientation: "landscape",
            header: {
                text: x1,
                alignment: "center",
                margin: [0, 80, 0, 0],
                fontSize: 16,
            },
            background: function (page) {

                return [
                    '\r\n',
                    {
                        image: 'bee',
                        width: 200
                    }
                ];

            },
            defaultStyle: {
                font: 'custom',
                alignment: 'center',
                top: 200
            },
        };
    
        pdfMake.createPdf(docDefinition).download();


    }

    $scope.createArr = function (list) {

        var arr = [];

        for (var i = 0; i <= list.length; i++) {
            arr[i] = [];
        }

        arr[0] = [
            {
                text: 'الاجمالي شامل.ض',
                alignment: 'center'
            },
            {
                text: 'اجمالي ض.',
                alignment: 'center'
            },
            {
                text: 'ض.المعفى',
                alignment: 'center'
            },
            {
                text: 'المعفى من ض.',
                alignment: 'center'
            },
            {
                text: 'ض.0%',
                alignment: 'center'
            },
            {
                text: 'خ.0%',
                alignment: 'center'
            },
            {
                text: 'ض.5%',
                alignment: 'center'
            },
            {
                text: 'خ.5%',
                alignment: 'center'
            },
            {
                text: 'الغير خاضع',
                alignment: 'center'
            },
            //{
            //    text: 'صافى الفاتورة',
            //    alignment: 'center'
            //},
            {
                text: 'الخصم',
                alignment: 'center'
            },
            {
                text: 'ج. قبل ض.',
                alignment: 'center'
            },
            {
                text: 'ع 18',
                alignment: 'center'
            },

            {
                text: 'ع 21',
                alignment: 'center'
            },
            {
                text: 'ع 22',
                alignment: 'center'
            },
            {
                text: 'ع 24',
                alignment: 'center'
            },
            {
                text: 'المناولة',
                alignment: 'center'
            },
            {
                text: 'ح. العميل',
                alignment: 'center'
            },
            {
                text: 'العميل',
                alignment: 'center'
            },
            {
                text: 'اسم السند',
                alignment: 'center'
            },
            {
                text: 'ر. السند',
                alignment: 'center'
            },

            {
                text: 'ت. السند',
                alignment: 'center'
            },
            {
                text: 'م',
                alignment: 'center'
            }
        ];

        for (var i = 0; i < 22; i++) {

            arr[0][i].text = arr[0][i].text.split(" ").reverse().join(" ");
        }

        var index = 1;

        for (var i = 0; i < list.length; i++) {
            arr[index][21] = i + 1;

            var obj = list[i];
            var j = 0;
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (key == "BILL_DATE") {
                        if (obj[key]) {
                            var d1 = new Date(obj[key]);
                            var billDate = d1.getFullYear() + "-" + ("0" + (d1.getMonth() + 1)).slice(-2) + "-" + ("0" + d1.getDate()).slice(-2);
                            arr[index][20] = billDate;
                        }
                        else {
                            arr[index][20] = "";
                        }
                    }
                    else if (key == "BILL_NUMBER") {
                        arr[index][19] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "BILL_ABRV_AR") {
                        arr[index][18] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "ACC_AR_NAME") {
                        arr[index][17] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "ACC_CODE") {
                        arr[index][16] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "DeliveryPersonName") {
                        arr[index][15] = obj[key] == undefined || obj[key] == null ? ' ' : obj[key];
                    }
                    else if (key == "TransTotalGold24") {
                        arr[index][14] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TransTotalGold22") {
                        arr[index][13] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TransTotalGold21") {
                        arr[index][12] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TransTotalGold18") {
                        arr[index][11] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalBeforeTax") {
                        arr[index][10] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalAllDisc") {
                        arr[index][9] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    //else if (key == "TotalMustPaid") {
                    //    arr[index][9] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    //}                   
                    else if (key == "TotalNotTaxableAmount") {
                        arr[index][8] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalMainVat") {
                        arr[index][7] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "MainVatValue") {
                        arr[index][6] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalZeroVat") {
                        arr[index][5] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "ZeroVatValue") {
                        arr[index][4] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalExemptOfTax") {
                        arr[index][3] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "ExemptOfTaxValue") {
                        arr[index][2] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalVatValue") {
                        arr[index][1] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    else if (key == "TotalAfterTax") {
                        arr[index][0] = obj[key] == undefined || obj[key] == null ? '0.00' : parseFloat(Number(obj[key])).toFixed(2);
                    }
                    j++;
                }
            }
            index++;
        }
        return arr;
    }    
}]);


