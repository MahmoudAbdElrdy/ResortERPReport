'use strict';
app.controller('ItemsInventoryController', ['$scope', '$location', '$log', '$q', '$filter', '$document', '$uibModal', 'ReportsService', '$uibModalStack', function ($scope, $location, $log, $q, $filter, $document, $uibModal, ReportsService, $uibModalStack) {
    $scope.ReportType = $location.search().ReportType == undefined || $location.search().ReportType == null ? 1 : $location.search().ReportType;
    $scope.ItemsInventory = {
        SortBy: '1',
        ShowZeroBalances: true,
        SellTypeId: 1
    };

    $scope.showGroup = false;
    $scope.DisablePrint = true;
    $scope.reportOptionList = [
        { NAME: "عرض المجموعات", ID: "1", SELECTED: false },
        { NAME: "تفاصيل المخازن", ID: "2", SELECTED: false },
        { NAME: "عرض تاريخ الصلاحية", ID: "3", SELECTED: false },
        { NAME: "اظهار الارصدة التى قيمتها صفر", ID: "4", SELECTED: true },
        { NAME: "اظهار الارصدة التى قيمتها سالب فقط", ID: "5", SELECTED: false },
        { NAME: "اظهار الارصدة التى قيمتها صفر فقط", ID: "6", SELECTED: false },
    ];
    $scope.billTypesList = [
        { NAME: "جملة ", ID: "1", SELECTED: false },
        { NAME: "نصف جملة ", ID: "2", SELECTED: false },
        { NAME: "قطاعى ", ID: "3", SELECTED: false },
        { NAME: "مستهلك ", ID: "4", SELECTED: false },
        { NAME: "مندوب ", ID: "5", SELECTED: false },
        { NAME: "تصدير ", ID: "6", SELECTED: false },
        { NAME: "اخر شراء ", ID: "7", SELECTED: false },
    ];
    $scope.PrintOptions = {
        ShowInputs: true,
        ShowOutputs: true,
        ShowTransfer: true,
        ShowEmptyItems: true,
        ShowBalance: true,
        PrintLandscape: false,
    };

    $scope.clearEnity = function () {
        $scope.ItemsInventory = {
            SortBy: '1',
            ShowZeroBalances: true,
            SellTypeId: 1
        };
        $scope.showGroup = false;
        $scope.DisablePrint = true;
        $scope.reportOptionList = [
            { NAME: "عرض المجموعات", ID: "1", SELECTED: false },
            { NAME: "تفاصيل المخازن", ID: "2", SELECTED: false },
            { NAME: "عرض تاريخ الصلاحية", ID: "3", SELECTED: false },
            { NAME: "اظهار الارصدة التى قيمتها صفر", ID: "4", SELECTED: true },
            { NAME: "اظهار الارصدة التى قيمتها سالب فقط", ID: "5", SELECTED: false },
            { NAME: "اظهار الارصدة التى قيمتها صفر فقط", ID: "6", SELECTED: false },
        ];
        $scope.billTypesList = [
            { NAME: "جملة ", ID: "1", SELECTED: false },
            { NAME: "نصف جملة ", ID: "2", SELECTED: false },
            { NAME: "قطاعى ", ID: "3", SELECTED: false },
            { NAME: "مستهلك ", ID: "4", SELECTED: false },
            { NAME: "مندوب ", ID: "5", SELECTED: false },
            { NAME: "تصدير ", ID: "6", SELECTED: false },
            { NAME: "اخر شراء ", ID: "7", SELECTED: false },
        ];
        $scope.PrintOptions = {
            ShowInputs: true,
            ShowOutputs: true,
            ShowTransfer: true,
            ShowEmptyItems: true,
            ShowBalance: true,
            PrintLandscape: false,
        };
        $scope.setReportDates();
        $scope.ReportResult = [];
    };

    $scope.getTafqeet = function(value) {
        if (value == 0 || value == undefined || value == null) {
            return "لا شيء";
        }
        else {
            return tafqeet(value) + " فقط لا غير ";
        }
    }

    $scope.TafqeetAllTotals=function() {
        $scope.ReportResult.TotalPrices.WHOLE_PRICETafqeet = $scope.getTafqeet($scope.ReportResult.TotalPrices.WHOLE_PRICE);
        $scope.ReportResult.TotalPrices.HALF_WHOLE_PRICETafqeet = $scope.getTafqeet($scope.ReportResult.TotalPrices.HALF_WHOLE_PRICE);
        $scope.ReportResult.TotalPrices.EMP_PRICETafqeet = $scope.getTafqeet($scope.ReportResult.TotalPrices.EMP_PRICE);
        $scope.ReportResult.TotalPrices.EXPORT_PRICETafqeet = $scope.getTafqeet($scope.ReportResult.TotalPrices.EXPORT_PRICE);
        $scope.ReportResult.TotalPrices.RETAIL_PRICETafqeet = $scope.getTafqeet($scope.ReportResult.TotalPrices.RETAIL_PRICE);
        $scope.ReportResult.TotalPrices.CONSUMER_PRICETafqeet = $scope.getTafqeet($scope.ReportResult.TotalPrices.CONSUMER_PRICE);
        $scope.ReportResult.TotalPrices.LAST_BUY_PRICETafqeet = $scope.getTafqeet($scope.ReportResult.TotalPrices.LAST_BUY_PRICE);
    }

    $scope.UserBranches = [];
    $scope.dropdownSetting = {
        scrollable: true,
        scrollableHeight: '300px',
        checkBoxes: true,
        enableSearch: true,
        buttonClasses: 'btn btn-default btn-block',
        styleActive: true,
        smartButtonTextProvider: function (selectedItems) {
            if (selectedItems.length > 3) {
                return "تم اختيار " + selectedItems.length + ' فروع';
            }
            else if (selectedItems.length == 3) {
                return selectedItems[0].label + ', ' + selectedItems[1].label + ', ' + selectedItems[2].label;
            }
            else if (selectedItems.length == 2) {
                return selectedItems[0].label + ', ' + selectedItems[1].label;
            }
            else if (selectedItems.length == 1) {
                return selectedItems[0].label;
            }
            else {
                return 'اختار فرع';
            }
        }
    }

    $scope.CheckBoxChange = function (item, index) {
        if (index == 0) {
            $scope.showGroup = item.SELECTED ? true : false;
        }
        else if (index == 1) {
            $scope.ItemsInventory.ShowStoreDetails = item.SELECTED ? true : false;
        }
        else if (index == 2) {
            $scope.ItemsInventory.ShowExpired = item.SELECTED ? true : false;
        }
        else if (index == 3) {
            $scope.ItemsInventory.ShowZeroBalances = item.SELECTED ? true : false;
        }
        else if (index == 4) {
            $scope.ItemsInventory.ShowNegativeBalancesOnly = item.SELECTED ? true : false;
            if (item.SELECTED) {
                $scope.reportOptionList[5].SELECTED = false;
                $scope.ItemsInventory.ShowZeroBalancesOnly = false;
            }
        }
        else if (index == 5) {
            $scope.ItemsInventory.ShowZeroBalancesOnly = item.SELECTED ? true : false;
            if (item.SELECTED) {
                $scope.reportOptionList[4].SELECTED = false;
                $scope.ItemsInventory.ShowNegativeBalancesOnly = false;
            }
        }
    }

    $scope.getTableWidthSpan = function () {
        var tableSpan = 4;
        if ($scope.ItemsInventory.ShowStoreDetails) {
            tableSpan += 1;
        }
        if ($scope.ItemsInventory.ShowExpired) {
            tableSpan += 1;
        }
        return tableSpan;
    };

    $scope.getBalancesTableWidthSpan = function () {
        var tableSpan = 5;
        if ($scope.ItemsInventory.ShowStoreDetails) {
            tableSpan += 1;
        }
        if ($scope.ItemsInventory.ShowExpired) {
            tableSpan += 1;
        }
        return tableSpan + ($filter('filter')($scope.SellTypeList, function (item) { return item.SELECTED === true && item.ID != $scope.ItemsInventory.SellTypeId })).length;
    };

    $scope.getTableWidths=function(size) {
        var arrayWidths = [];
        for (var i = 0; i < size; i++) {
            arrayWidths.push("*");
        }
        return arrayWidths;
    }

    $scope.getBranchesList = function () {
        ReportsService.getBranchesList().then(function (response) {
            $scope.BranchesList = response.data;
        }, function (error) { console.log(error.data); });
    };

    $scope.getCostCenterList = function () {
        ReportsService.getCostCenterList().then(function (response) {
            $scope.CostCenterList = response.data;
        }, function (error) { console.log(error.data); });
    };

    $scope.getStoresList = function () {
        ReportsService.getStoresList().then(function (response) {
            $scope.StoresList = response.data;
        }, function (error) { console.log(error.data); });
    };

    $scope.getItemGroupList = function () {
        ReportsService.getItemGroupList().then(function (response) {
            $scope.ItemGroupList = response.data;
        }, function (error) { console.log(error.data); });
    };

    $scope.getSellTypeList = function () {
        ReportsService.getSellTypeList().then(function (response) {
            $scope.SellTypeList = response.data;
        }, function (error) { console.log(error.data); });
    };

    $scope.getAllOnLoad = function () {
        $q.all(
            [
                $scope.setReportDates(),
                $scope.getBranchesList(),
                $scope.getCostCenterList(),
                $scope.getStoresList(),
                $scope.getItemGroupList(),
                $scope.getSellTypeList(),
            ]).then(function (allResponses) {
            }, function (error) {
            });
    }

    $scope.setReportDates = function () {
        var date = new Date();
        $scope.ItemsInventory.dateFrom = new Date(2000, 0, 1);
        $scope.ItemsInventory.dateTo = date;
    }

    $scope.GetSearchResult = function () {
            ReportsService.getItemsInventoryReportResult($scope.ItemsInventory).then(function (result) {
                if (result && result.data) {
                    var GroupSelected = $scope.reportOptionList[0].SELECTED;
                    $scope.ReportResult = result.data;
                    ReportsService.getItemsInventoryBalanceResult($scope.ItemsInventory).then(function (balance) {
                        if (balance && balance.data) {
                            $scope.ReportResult.ItemsBalance = balance.data.ItemsBalance;
                            $scope.ReportResult.TotalPrices = balance.data.TotalPrices;
                            $scope.TafqeetAllTotals();
                        }
                    })
                }
            }, function (error) {
                console.log(error);
            });       
        $scope.DisablePrint = false;
    };

    $scope.PrintReport = function () {
        pdfMake.fonts = {
            custom: {
                normal: 'DroidKufi-Regular.ttf'
            }
        }

        var tableLayout = {
            hLineWidth: function (i, node) {
                if (i === 0 || i === node.table.body.length) {
                    return 0;
                }
                return (i === node.table.headerRows) ? 2 : 1;
            },
            vLineWidth: function (i) {
                return 0;
            },
            hLineColor: function (i) {
                return i === 1 ? 'black' : '#aaa';
            },
            paddingLeft: function (i) {
                return i === 0 ? 0 : 8;
            },
            paddingRight: function (i, node) {
                return (i === node.table.widths.length - 1) ? 0 : 8;
            },
            fillColor: function (i, node) {
                return i % 2 == 0 ? 'white' : '#eee';
            }
        };

        var docDefinition = {
            content: [],
            styles: {
                subheader: {
                    fontSize: 16,
                }
            },
            images: {
                bee: "data:image/png;base64,"
            },
            pageMargins: [20, 150, 20, 0],
            pageOrientation: $scope.PrintOptions.PrintLandscape ? "landscape" : "portrait",
            pageSize: 'A3',
            header: {
                text: "تقرير جرد الاصناف".split(" ").reverse().join(" "),
                alignment: "center",
                margin: [0, 80, 0, 0],
                fontSize: 16,
            },
            background: function (page) {

                return [
                    '\r\n',
                    {
                        image: 'bee',
                        width: 200
                    }
                ];

            },
            defaultStyle: {
                font: 'custom',
                alignment: 'center',
                top: 200
            },
        };

        if ($scope.PrintOptions.ShowInputs && $scope.ReportType == 2) {
            docDefinition.content.push([
                {text: 'الادخالات', style: 'header' },
                {
                    table: {
                        headerRows: 1,
                        widths: $scope.getTableWidths($scope.getTableWidthSpan()),
                        body: $scope.CreateInputsTableBody()
                    },
                    layout: tableLayout                    
                }
            ])
        }
        if ($scope.PrintOptions.ShowOutputs && $scope.ReportType == 2) {
            docDefinition.content.push([
                { text: 'الاخراجات', style: 'header', style: 'subheader'},
                {
                    table: {
                        headerRows: 1,
                        widths: $scope.getTableWidths($scope.getTableWidthSpan()),
                        body: $scope.CreateOutputsTableBody()
                    },
                    layout: tableLayout
                }
            ])
        }
        if ($scope.PrintOptions.ShowTransfer && $scope.ReportType == 2) {
            docDefinition.content.push([
                { text: 'التحويلات', style: 'header', style: 'subheader'},
                {
                    table: {
                        headerRows: 1,
                        widths: $scope.getTableWidths($scope.getTableWidthSpan()),
                        body: $scope.CreateTransferTableBody()
                    },
                    layout: tableLayout
                }
            ])
        }
        if ($scope.PrintOptions.ShowEmptyItems && $scope.ReportType == 2) {
            docDefinition.content.push([
                { text: 'الاصناف الفارغة'.split(" ").reverse().join(" "), style: 'header', style: 'subheader'},
                {
                    table: {
                        headerRows: 1,
                        widths: $scope.getTableWidths($scope.getTableWidthSpan() - 1),
                        body: $scope.CreateEmptyItemsTableBody()
                    },
                    layout: tableLayout
                }
            ])
        }
        if ($scope.PrintOptions.ShowBalance) {
            docDefinition.content.push([
                { text: 'الرصيد', style: 'header', style: 'subheader' },
                {
                    table: {
                        headerRows: 1,
                        widths: $scope.getTableWidths($scope.getBalancesTableWidthSpan()),
                        body: $scope.CreateItemsBalanceTableBody()
                    },
                    layout: tableLayout
                }
            ])
        }

        pdfMake.createPdf(docDefinition).download();

    }

    // Inputs Table
    $scope.CreateInputsTableBody = function () {
        var tableData = [];
        tableData[0] = [];
        if ($scope.ItemsInventory.ShowExpired) {
            tableData[0].push({ text: "تاريخ الصلاحية".split(" ").reverse().join(" ") });
        }
        if ($scope.ItemsInventory.ShowStoreDetails) {
            tableData[0].push({ text: "المخزن".split(" ").reverse().join(" ") });
        }
        tableData[0].push(
            {
                text: "الكمية",
            },
            {
                text: "الاسم",
            },
            {
                text: "الكود",
            },
            {
                text: "مسلسل",
            },
        );
        if ($scope.showGroup) {
            var index = 0;
            for (var k = 0; k < $scope.ReportResult.InputsOnGroups.length; k++) {
                index++;
                tableData[index] = [{
                    text: $scope.ReportResult.InputsOnGroups[k].GROUP_AR_NAME != undefined && $scope.ReportResult.InputsOnGroups[k].GROUP_AR_NAME != null ?
                        $scope.ReportResult.InputsOnGroups[k].GROUP_AR_NAME.split(" ").reverse().join(" ") : "",
                    colSpan: $scope.getTableWidthSpan()
                }];
                var Inputs = $filter('filter')($scope.ReportResult.Inputs, function (item) {
                    return item.GROUP_ID === $scope.ReportResult.InputsOnGroups[k].GROUP_ID
                });
                for (var i = 0; i < Inputs.length; i++) {
                    index++;
                    tableData[index] = [];
                    if ($scope.ItemsInventory.ShowExpired) {
                        tableData[index].push({
                            text: Inputs[i].EXPIRED_DATE != undefined && Inputs[i].EXPIRED_DATE != null
                                ? Inputs[i].EXPIRED_DATE : "",
                        });
                    }
                    if ($scope.ItemsInventory.ShowStoreDetails) {
                        tableData[index].push({
                            text: Inputs[i].COM_STORE_ID != undefined && Inputs[i].COM_STORE_ID != null
                                ? Inputs[i].COM_STORE_ID
                                : "",
                        });
                    }
                    tableData[index].push(
                        {
                            text: Inputs[i].QTY != undefined && Inputs[i].QTY != null ? Inputs[i].QTY : "",
                        },
                        {
                            text: Inputs[i].ITEM_AR_NAME != undefined && Inputs[i].ITEM_AR_NAME != null
                                ? Inputs[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                                : "",
                        },
                        {
                            text: Inputs[i].ITEM_CODE != undefined && Inputs[i].ITEM_CODE != null
                                ? Inputs[i].ITEM_CODE
                                : "",
                        },
                        {
                            text: i + 1
                        }
                    );
                }
            }
        }
        else {
            for (var i = 0; i < $scope.ReportResult.Inputs.length; i++) {
                tableData[i + 1] = [];
                if ($scope.ItemsInventory.ShowExpired) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.Inputs[i].EXPIRED_DATE != undefined && $scope.ReportResult.Inputs[i].EXPIRED_DATE != null
                            ? $scope.ReportResult.Inputs[i].EXPIRED_DATE : "",
                    });
                }
                if ($scope.ItemsInventory.ShowStoreDetails) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.Inputs[i].COM_STORE_ID != undefined && $scope.ReportResult.Inputs[i].COM_STORE_ID != null
                            ? $scope.ReportResult.Inputs[i].COM_STORE_ID
                            : "",
                    });
                }
                tableData[i + 1].push(
                    {
                        text: $scope.ReportResult.Inputs[i].QTY != undefined &&
                            $scope.ReportResult.Inputs[i].QTY != null
                            ? $scope.ReportResult.Inputs[i].QTY
                            : "",
                    },
                    {
                        text: $scope.ReportResult.Inputs[i].ITEM_AR_NAME != undefined &&
                            $scope.ReportResult.Inputs[i].ITEM_AR_NAME != null
                            ? $scope.ReportResult.Inputs[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                            : "",
                    },
                    {
                        text: $scope.ReportResult.Inputs[i].ITEM_CODE != undefined &&
                            $scope.ReportResult.Inputs[i].ITEM_CODE != null
                            ? $scope.ReportResult.Inputs[i].ITEM_CODE
                            : "",
                    },
                    {
                        text: i + 1
                    }
                );
            }
        }
        return tableData;
    }

    // Outputs Table
    $scope.CreateOutputsTableBody = function () {
        var tableData = [];
        tableData[0] = [];
        if ($scope.ItemsInventory.ShowExpired) {
            tableData[0].push({ text: "تاريخ الصلاحية".split(" ").reverse().join(" ") });
        }
        if ($scope.ItemsInventory.ShowStoreDetails) {
            tableData[0].push({ text: "المخزن".split(" ").reverse().join(" ") });
        }
        tableData[0].push(
            {
                text: "الكمية",
            },
            {
                text: "الاسم",
            },
            {
                text: "الكود",
            },
            {
                text: "مسلسل",
            },
        );
        if ($scope.showGroup) {
            var index = 0;
            for (var k = 0; k < $scope.ReportResult.OutputOnGroups.length; k++) {
                index++;
                tableData[index] = [{
                    text: $scope.ReportResult.OutputOnGroups[k].GROUP_AR_NAME != undefined && $scope.ReportResult.OutputOnGroups[k].GROUP_AR_NAME != null ?
                        $scope.ReportResult.OutputOnGroups[k].GROUP_AR_NAME.split(" ").reverse().join(" ") : "",
                    colSpan: $scope.getTableWidthSpan()
                }];
                var Outputs = $filter('filter')($scope.ReportResult.Outputs, function (item) {
                    return item.GROUP_ID === $scope.ReportResult.OutputOnGroups[k].GROUP_ID
                });
                for (var i = 0; i < Outputs.length; i++) {
                    index++;
                    tableData[index] = [];
                    if ($scope.ItemsInventory.ShowExpired) {
                        tableData[index].push({
                            text: Outputs[i].EXPIRED_DATE != undefined && Outputs[i].EXPIRED_DATE != null
                                ? Outputs[i].EXPIRED_DATE : "",
                        });
                    }
                    if ($scope.ItemsInventory.ShowStoreDetails) {
                        tableData[index].push({
                            text: Outputs[i].COM_STORE_ID != undefined && Outputs[i].COM_STORE_ID != null
                                ? Outputs[i].COM_STORE_ID
                                : "",
                        });
                    }
                    tableData[index].push(
                        {
                            text: Outputs[i].QTY != undefined && Outputs[i].QTY != null ? Outputs[i].QTY : "",
                        },
                        {
                            text: Outputs[i].ITEM_AR_NAME != undefined && Outputs[i].ITEM_AR_NAME != null
                                ? Outputs[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                                : "",
                        },
                        {
                            text: Outputs[i].ITEM_CODE != undefined && Outputs[i].ITEM_CODE != null
                                ? Outputs[i].ITEM_CODE
                                : "",
                        },
                        {
                            text: i + 1
                        }
                    );
                }
            }
        }
        else {
            for (var i = 0; i < $scope.ReportResult.Outputs.length; i++) {
                tableData[i + 1] = [];
                if ($scope.ItemsInventory.ShowExpired) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.Outputs[i].EXPIRED_DATE != undefined && $scope.ReportResult.Outputs[i].EXPIRED_DATE != null
                            ? $scope.ReportResult.Outputs[i].EXPIRED_DATE : "",
                    });
                }
                if ($scope.ItemsInventory.ShowStoreDetails) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.Outputs[i].COM_STORE_ID != undefined && $scope.ReportResult.Outputs[i].COM_STORE_ID != null
                            ? $scope.ReportResult.Outputs[i].COM_STORE_ID
                            : "",
                    });
                }
                tableData[i + 1].push(
                    {
                        text: $scope.ReportResult.Outputs[i].QTY != undefined &&
                            $scope.ReportResult.Outputs[i].QTY != null
                            ? $scope.ReportResult.Outputs[i].QTY
                            : "",
                    },
                    {
                        text: $scope.ReportResult.Outputs[i].ITEM_AR_NAME != undefined &&
                            $scope.ReportResult.Outputs[i].ITEM_AR_NAME != null
                            ? $scope.ReportResult.Outputs[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                            : "",
                    },
                    {
                        text: $scope.ReportResult.Outputs[i].ITEM_CODE != undefined &&
                            $scope.ReportResult.Outputs[i].ITEM_CODE != null
                            ? $scope.ReportResult.Outputs[i].ITEM_CODE
                            : "",
                    },
                    {
                        text: i + 1
                    }
                );
            }
        }
        return tableData;
    }

    // Transfer Table
    $scope.CreateTransferTableBody = function () {
        var tableData = [];
        tableData[0] = [];
        if ($scope.ItemsInventory.ShowExpired) {
            tableData[0].push({ text: "تاريخ الصلاحية".split(" ").reverse().join(" ") });
        }
        if ($scope.ItemsInventory.ShowStoreDetails) {
            tableData[0].push({ text: "المخزن".split(" ").reverse().join(" ") });
        }
        tableData[0].push(
            {
                text: "الكمية",
            },
            {
                text: "الاسم",
            },
            {
                text: "الكود",
            },
            {
                text: "مسلسل",
            },
        );
        if ($scope.showGroup) {
            var index = 0;
            for (var k = 0; k < $scope.ReportResult.TransferOnGroups.length; k++) {
                index++;
                tableData[index] = [{
                    text: $scope.ReportResult.TransferOnGroups[k].GROUP_AR_NAME != undefined && $scope.ReportResult.TransferOnGroups[k].GROUP_AR_NAME != null ?
                        $scope.ReportResult.TransferOnGroups[k].GROUP_AR_NAME.split(" ").reverse().join(" ") : "",
                    colSpan: $scope.getTableWidthSpan()
                }];
                var Transfer = $filter('filter')($scope.ReportResult.Transfer, function (item) {
                    return item.GROUP_ID === $scope.ReportResult.TransferOnGroups[k].GROUP_ID
                });
                for (var i = 0; i < Transfer.length; i++) {
                    index++;
                    tableData[index] = [];
                    if ($scope.ItemsInventory.ShowExpired) {
                        tableData[index].push({
                            text: Transfer[i].EXPIRED_DATE != undefined && Transfer[i].EXPIRED_DATE != null
                                ? Transfer[i].EXPIRED_DATE : "",
                        });
                    }
                    if ($scope.ItemsInventory.ShowStoreDetails) {
                        tableData[index].push({
                            text: Transfer[i].COM_STORE_ID != undefined && Transfer[i].COM_STORE_ID != null
                                ? Transfer[i].COM_STORE_ID
                                : "",
                        });
                    }
                    tableData[index].push(
                        {
                            text: Transfer[i].QTY != undefined && Transfer[i].QTY != null ? Transfer[i].QTY : "",
                        },
                        {
                            text: Transfer[i].ITEM_AR_NAME != undefined && Transfer[i].ITEM_AR_NAME != null
                                ? Transfer[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                                : "",
                        },
                        {
                            text: Transfer[i].ITEM_CODE != undefined && Transfer[i].ITEM_CODE != null
                                ? Transfer[i].ITEM_CODE
                                : "",
                        },
                        {
                            text: i + 1
                        }
                    );
                }
            }
        }
        else {
            for (var i = 0; i < $scope.ReportResult.Transfer.length; i++) {
                tableData[i + 1] = [];
                if ($scope.ItemsInventory.ShowExpired) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.Transfer[i].EXPIRED_DATE != undefined && $scope.ReportResult.Transfer[i].EXPIRED_DATE != null
                            ? $scope.ReportResult.Transfer[i].EXPIRED_DATE : "",
                    });
                }
                if ($scope.ItemsInventory.ShowStoreDetails) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.Transfer[i].COM_STORE_ID != undefined && $scope.ReportResult.Transfer[i].COM_STORE_ID != null
                            ? $scope.ReportResult.Transfer[i].COM_STORE_ID
                            : "",
                    });
                }
                tableData[i + 1].push(
                    {
                        text: $scope.ReportResult.Transfer[i].QTY != undefined &&
                            $scope.ReportResult.Transfer[i].QTY != null
                            ? $scope.ReportResult.Transfer[i].QTY
                            : "",
                    },
                    {
                        text: $scope.ReportResult.Transfer[i].ITEM_AR_NAME != undefined &&
                            $scope.ReportResult.Transfer[i].ITEM_AR_NAME != null
                            ? $scope.ReportResult.Transfer[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                            : "",
                    },
                    {
                        text: $scope.ReportResult.Transfer[i].ITEM_CODE != undefined &&
                            $scope.ReportResult.Transfer[i].ITEM_CODE != null
                            ? $scope.ReportResult.Transfer[i].ITEM_CODE
                            : "",
                    },
                    {
                        text: i + 1
                    }
                );
            }
        }
        return tableData;
    }

    // Empty Items Table
    $scope.CreateEmptyItemsTableBody = function () {
        var tableData = [];
        tableData[0] = [];
        if ($scope.ItemsInventory.ShowExpired) {
            tableData[0].push({ text: "تاريخ الصلاحية".split(" ").reverse().join(" ") });
        }
        if ($scope.ItemsInventory.ShowStoreDetails) {
            tableData[0].push({ text: "المخزن".split(" ").reverse().join(" ") });
        }
        tableData[0].push(
            {
                text: "الاسم",
            },
            {
                text: "الكود",
            },
            {
                text: "مسلسل",
            },
        );
        if ($scope.showGroup) {
            var index = 0;
            for (var k = 0; k < $scope.ReportResult.EmptyItemsOnGroups.length; k++) {
                index++;
                tableData[index] = [{
                    text: $scope.ReportResult.EmptyItemsOnGroups[k].GROUP_AR_NAME != undefined && $scope.ReportResult.EmptyItemsOnGroups[k].GROUP_AR_NAME != null ?
                        $scope.ReportResult.EmptyItemsOnGroups[k].GROUP_AR_NAME.split(" ").reverse().join(" ") : "",
                    colSpan: $scope.getTableWidthSpan() - 1
                }];
                var EmptyItems = $filter('filter')($scope.ReportResult.EmptyItems, function (item) {
                    return item.GROUP_ID === $scope.ReportResult.EmptyItemsOnGroups[k].GROUP_ID
                });
                for (var i = 0; i < EmptyItems.length; i++) {
                    index++;
                    tableData[index] = [];
                    if ($scope.ItemsInventory.ShowExpired) {
                        tableData[index].push({
                            text: EmptyItems[i].EXPIRED_DATE != undefined && EmptyItems[i].EXPIRED_DATE != null
                                ? EmptyItems[i].EXPIRED_DATE : "",
                        });
                    }
                    if ($scope.ItemsInventory.ShowStoreDetails) {
                        tableData[index].push({
                            text: EmptyItems[i].COM_STORE_ID != undefined && EmptyItems[i].COM_STORE_ID != null
                                ? EmptyItems[i].COM_STORE_ID
                                : "",
                        });
                    }
                    tableData[index].push(
                        {
                            text: EmptyItems[i].ITEM_AR_NAME != undefined && EmptyItems[i].ITEM_AR_NAME != null
                                ? EmptyItems[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                                : "",
                        },
                        {
                            text: EmptyItems[i].ITEM_CODE != undefined && EmptyItems[i].ITEM_CODE != null
                                ? EmptyItems[i].ITEM_CODE
                                : "",
                        },
                        {
                            text: i + 1
                        }
                    );
                }
            }
        }
        else {
            for (var i = 0; i < $scope.ReportResult.EmptyItems.length; i++) {
                tableData[i + 1] = [];
                if ($scope.ItemsInventory.ShowExpired) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.EmptyItems[i].EXPIRED_DATE != undefined && $scope.ReportResult.EmptyItems[i].EXPIRED_DATE != null
                            ? $scope.ReportResult.EmptyItems[i].EXPIRED_DATE : "",
                    });
                }
                if ($scope.ItemsInventory.ShowStoreDetails) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.EmptyItems[i].COM_STORE_ID != undefined && $scope.ReportResult.EmptyItems[i].COM_STORE_ID != null
                            ? $scope.ReportResult.EmptyItems[i].COM_STORE_ID
                            : "",
                    });
                }
                tableData[i + 1].push(
                    //{
                    //    text: $scope.ReportResult.EmptyItems[i].QTY != undefined && $scope.ReportResult.EmptyItems[i].QTY != null ?
                    //        $scope.ReportResult.EmptyItems[i].QTY : "",
                    //},
                    {
                        text: $scope.ReportResult.EmptyItems[i].ITEM_AR_NAME != undefined &&
                            $scope.ReportResult.EmptyItems[i].ITEM_AR_NAME != null
                            ? $scope.ReportResult.EmptyItems[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                            : "",
                    },
                    {
                        text: $scope.ReportResult.EmptyItems[i].ITEM_CODE != undefined &&
                            $scope.ReportResult.EmptyItems[i].ITEM_CODE != null
                            ? $scope.ReportResult.EmptyItems[i].ITEM_CODE
                            : "",
                    },
                    {
                        text: i + 1
                    }
                );
            }
        }
        return tableData;
    }

    // Balance Table
    $scope.CreateItemsBalanceTableBody = function () {
        var tableData = [];
        tableData[0] = [];
        if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 7 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 7) {
            tableData[0].push({ text: "سعر اخر شراء".split(" ").reverse().join(" ") });
        }
        if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 6 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 6) {
            tableData[0].push({ text: "سعر تصدير".split(" ").reverse().join(" ") });
        }
        if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 5 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 5) {
            tableData[0].push({ text: "سعر مندوب".split(" ").reverse().join(" ") });
        }
        if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 4 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 4) {
            tableData[0].push({ text: "سعر مستهلك".split(" ").reverse().join(" ") });
        }
        if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 3 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 3) {
            tableData[0].push({ text: "سعر قطاعى".split(" ").reverse().join(" ") });
        }
        if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 2 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 2) {
            tableData[0].push({ text: "سعر نصف جملة".split(" ").reverse().join(" ") });
        }
        if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 1 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 1) {
            tableData[0].push({ text: "سعر الجملة".split(" ").reverse().join(" ") });
        }        
        if ($scope.ItemsInventory.ShowExpired) {
            tableData[0].push({ text: "تاريخ الصلاحية".split(" ").reverse().join(" ") });
        }
        if ($scope.ItemsInventory.ShowStoreDetails) {
            tableData[0].push({ text: "المخزن".split(" ").reverse().join(" ") });
        }
        tableData[0].push(
            {
                text: "الكمية",
            },
            {
                text: "الاسم",
            },
            {
                text: "الكود",
            },
            {
                text: "مسلسل",
            },
        );
        if ($scope.showGroup) {
            var index = 0;
            for (var k = 0; k < $scope.ReportResult.ItemsBalanceOnGroup.length; k++) {
                index++;
                tableData[index] = [{
                    text: $scope.ReportResult.ItemsBalanceOnGroup[k].GROUP_AR_NAME != undefined && $scope.ReportResult.ItemsBalanceOnGroup[k].GROUP_AR_NAME != null ?
                        $scope.ReportResult.ItemsBalanceOnGroup[k].GROUP_AR_NAME.split(" ").reverse().join(" ") : "",
                    colSpan: $scope.getBalancesTableWidthSpan()
                }];
                var ItemsBalance = $filter('filter')($scope.ReportResult.ItemsBalance, function (item) {
                    return item.GROUP_ID === $scope.ReportResult.ItemsBalanceOnGroup[k].GROUP_ID
                });
                for (var i = 0; i < ItemsBalance.length; i++) {
                    index++;
                    tableData[index] = [];
                    if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 7 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 7) {
                        tableData[index].push({
                            text: ItemsBalance[i].LAST_BUY_PRICEQTY != undefined && ItemsBalance[i].LAST_BUY_PRICEQTY != null
                                ? ItemsBalance[i].LAST_BUY_PRICEQTY : "0",
                        });
                    }
                    if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 6 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 6) {
                        tableData[index].push({
                            text: ItemsBalance[i].EXPORT_PRICEQTY != undefined && ItemsBalance[i].EXPORT_PRICEQTY != null
                                ? ItemsBalance[i].EXPORT_PRICEQTY : "0",
                        });
                    }
                    if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 5 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 5) {
                        tableData[index].push({
                            text: ItemsBalance[i].EMP_PRICEQTY != undefined && ItemsBalance[i].EMP_PRICEQTY != null
                                ? ItemsBalance[i].EMP_PRICEQTY : "0",
                        });
                    }
                    if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 4 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 4) {
                        tableData[index].push({
                            text: ItemsBalance[i].CONSUMER_PRICEQTY != undefined && ItemsBalance[i].CONSUMER_PRICEQTY != null
                                ? ItemsBalance[i].CONSUMER_PRICEQTY : "0",
                        });
                    }
                    if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 3 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 3) {
                        tableData[index].push({
                            text: ItemsBalance[i].RETAIL_PRICEQTY != undefined && ItemsBalance[i].RETAIL_PRICEQTY != null
                                ? ItemsBalance[i].RETAIL_PRICEQTY : "0",
                        });
                    }
                    if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 2 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 2) {
                        tableData[index].push({
                            text: ItemsBalance[i].HALF_WHOLE_PRICEQTY != undefined && ItemsBalance[i].HALF_WHOLE_PRICEQTY != null
                                ? ItemsBalance[i].HALF_WHOLE_PRICEQTY : "0",
                        });
                    }
                    if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 1 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 1) {
                        tableData[index].push({
                            text: ItemsBalance[i].WHOLE_PRICEQTY != undefined && ItemsBalance[i].WHOLE_PRICEQTY != null
                                ? ItemsBalance[i].WHOLE_PRICEQTY : "0",
                        });
                    }

                    if ($scope.ItemsInventory.ShowExpired) {
                        tableData[index].push({
                            text: ItemsBalance[i].EXPIRED_DATE != undefined && ItemsBalance[i].EXPIRED_DATE != null
                                ? ItemsBalance[i].EXPIRED_DATE : "",
                        });
                    }
                    if ($scope.ItemsInventory.ShowStoreDetails) {
                        tableData[index].push({
                            text: ItemsBalance[i].COM_STORE_ID != undefined && ItemsBalance[i].COM_STORE_ID != null
                                ? ItemsBalance[i].COM_STORE_ID
                                : "",
                        });
                    }
                    tableData[index].push(
                        {
                            text: ItemsBalance[i].QTY != undefined && ItemsBalance[i].QTY != null
                                ? ItemsBalance[i].QTY
                                : "",
                        },
                        {
                            text: ItemsBalance[i].ITEM_AR_NAME != undefined && ItemsBalance[i].ITEM_AR_NAME != null
                                ? ItemsBalance[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                                : "",
                        },
                        {
                            text: ItemsBalance[i].ITEM_CODE != undefined && ItemsBalance[i].ITEM_CODE != null
                                ? ItemsBalance[i].ITEM_CODE
                                : "",
                        },
                        {
                            text: i + 1
                        }
                    );
                }
            }
        }
        else {
            for (var i = 0; i < $scope.ReportResult.ItemsBalance.length; i++) {
                tableData[i + 1] = [];
                if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 7 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 7) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].LAST_BUY_PRICEQTY != undefined && $scope.ReportResult.ItemsBalance[i].LAST_BUY_PRICEQTY != null
                            ? $scope.ReportResult.ItemsBalance[i].LAST_BUY_PRICEQTY : "0",
                    });
                }
                if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 6 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 6) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].EXPORT_PRICEQTY != undefined && $scope.ReportResult.ItemsBalance[i].EXPORT_PRICEQTY != null
                            ? $scope.ReportResult.ItemsBalance[i].EXPORT_PRICEQTY : "0",
                    });
                }
                if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 5 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 5) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].EMP_PRICEQTY != undefined && $scope.ReportResult.ItemsBalance[i].EMP_PRICEQTY != null
                            ? $scope.ReportResult.ItemsBalance[i].EMP_PRICEQTY : "0",
                    });
                }
                if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 4 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 4) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].CONSUMER_PRICEQTY != undefined && $scope.ReportResult.ItemsBalance[i].CONSUMER_PRICEQTY != null
                            ? $scope.ReportResult.ItemsBalance[i].CONSUMER_PRICEQTY : "0",
                    });
                }
                if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 3 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 3) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].RETAIL_PRICEQTY != undefined && $scope.ReportResult.ItemsBalance[i].RETAIL_PRICEQTY != null
                            ? $scope.ReportResult.ItemsBalance[i].RETAIL_PRICEQTY : "0",
                    });
                }
                if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 2 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 2) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].HALF_WHOLE_PRICEQTY != undefined && $scope.ReportResult.ItemsBalance[i].HALF_WHOLE_PRICEQTY != null
                            ? $scope.ReportResult.ItemsBalance[i].HALF_WHOLE_PRICEQTY : "0",
                    });
                }
                if (($filter('filter')($scope.SellTypeList, function (item) { return item.OptionValue === 1 }))[0].SELECTED || $scope.ItemsInventory.SellTypeId == 1) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].WHOLE_PRICEQTY != undefined && $scope.ReportResult.ItemsBalance[i].WHOLE_PRICEQTY != null
                            ? $scope.ReportResult.ItemsBalance[i].WHOLE_PRICEQTY : "0",
                    });
                }
                if ($scope.ItemsInventory.ShowExpired) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].EXPIRED_DATE != undefined && $scope.ReportResult.ItemsBalance[i].EXPIRED_DATE != null
                            ? $scope.ReportResult.ItemsBalance[i].EXPIRED_DATE : "",
                    });
                }
                if ($scope.ItemsInventory.ShowStoreDetails) {
                    tableData[i + 1].push({
                        text: $scope.ReportResult.ItemsBalance[i].COM_STORE_ID != undefined && $scope.ReportResult.ItemsBalance[i].COM_STORE_ID != null
                            ? $scope.ReportResult.ItemsBalance[i].COM_STORE_ID
                            : "",
                    });
                }
                tableData[i + 1].push(
                    {
                        text: $scope.ReportResult.ItemsBalance[i].QTY != undefined &&
                            $scope.ReportResult.ItemsBalance[i].QTY != null
                            ? $scope.ReportResult.ItemsBalance[i].QTY
                            : "",
                    },
                    {
                        text: $scope.ReportResult.ItemsBalance[i].ITEM_AR_NAME != undefined &&
                            $scope.ReportResult.ItemsBalance[i].ITEM_AR_NAME != null
                            ? $scope.ReportResult.ItemsBalance[i].ITEM_AR_NAME.split(" ").reverse().join(" ")
                            : "",
                    },
                    {
                        text: $scope.ReportResult.ItemsBalance[i].ITEM_CODE != undefined &&
                            $scope.ReportResult.ItemsBalance[i].ITEM_CODE != null
                            ? $scope.ReportResult.ItemsBalance[i].ITEM_CODE
                            : "",
                    },
                    {
                        text: i + 1
                    }
                );
            }
        }
        return tableData;
    }

}]);