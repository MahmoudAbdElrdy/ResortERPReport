...
app.controller('GoldBoxMotionMonthlyController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementGoldMonthlyService', 'compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementGoldMonthlyService, compBranchesService) {
    ...
    $scope.fnExcelReport = function () {
        ...
        var x1 = "حركة صندوق الذهب المشغول (شهرية )".split(" ").reverse().join(" ");
                ...
                if (obj.hasOwnProperty(key)) {
                    if (key == "BAL_Gold24") {
                        ...
                        if (obj[key]) {
                            ...
                            arr[index][16] = obj[key]; //account_date
                        }
                        else {
                            arr[index][16] = "";
                        }
