'use strict';
app.controller('TotalBillsMotionController', ['$scope', '$location', '$log', '$q', 'billSettingService', 'customersService', 'companyStoresService', 'currencyService', 'costCentersService', 'employeesService', 'billProfitReportPageService', '$document', '$uibModal', '$uibModalStack', 'TotalBillsMotionService', 'payTypesService','compBranchesService', function ($scope, $location, $log, $q, billSettingService, customersService, companyStoresService, currencyService, costCentersService, employeesService, profitService, $document, $uibModal, $uibModalStack, TotalBillsMotionService, payTypesService, compBranchesService) {
    ...
    $scope.GetSearchResult = function () {
        debugger;//&& $scope.search.customerName !== null && $scope.search.customerName !== undefined
        //$scope.search.dateFrom = new Date($scope.search.dateFrom);
        if ($scope.search !== undefined && $scope.search !== null && $scope.SelectedBranch.length > 0) {
            ...
            for (var i = 0; i < $scope.billSettingsList.length; i++) {
                if ($scope.billSettingsList[i].selected === true) {
                    $scope.selectedbillSettingsList.push($scope.billSettingsList[i].BILL_SETTING_ID);
                }
            }
            if ($scope.selectedbillSettingsList.length > 0) {
                ...
                for (var i = 0; i < $scope.reportOptionList.length; i++) {
                    if ($scope.reportOptionList[i].SELECTED === true) {
                        $scope.selectedReportOptionList.push($scope.reportOptionList[i].ID);
                    }
                }
                $scope.search.bilSettings = $scope.selectedbillSettingsList;
                $scope.search.rptOptions = $scope.selectedReportOptionList;

                TotalBillsMotionService.getSearch($scope.search).then(
                    function (response) {
    ...
    $scope.GettingSearchResult = function (entity) {
        if (entity !== undefined && entity !== null) {
            var quantity = 0;
            $scope.searchList = entity;
            $scope.totalPaidValue = 0;
            $scope.totalRemainValue = 0;
            $scope.TotalBillResult = 0;            
            for (var i = 0; i < entity.length; i++) {
                if (entity[i].TotalPaid != undefined && entity[i].TotalPaid != null && entity[i].TotalPaid != "") {
                    $scope.totalPaidValue += parseFloat(entity[i].TotalPaid);
                }
                if (entity[i].TotalRemaining != undefined && entity[i].TotalRemaining != null && entity[i].TotalRemaining != "") {
                    $scope.totalRemainValue += parseFloat(entity[i].TotalRemaining);
                }
                if (entity[i].BILL_TOTAL != undefined && entity[i].BILL_TOTAL != null && entity[i].BILL_TOTAL != "") {
                    $scope.TotalBillResult += parseFloat(entity[i].BILL_TOTAL);
                }
            }
            $scope.totalPaidValue = $scope.totalPaidValue.toFixed(2);
            $scope.totalRemainValue = $scope.totalRemainValue.toFixed(2);
            $scope.TotalBillResult = $scope.TotalBillResult.toFixed(2);
    ...
    $scope.getCurrentDate = function () {
        ...
        $scope.search.StartDate = date;
        $scope.search.EndDate = date;
    }
    ...
    $scope.searchForCustomer = function () {
        var search = $scope.search.customerName;
        search = search == undefined || search == "" ? "" : search;
        //if (search != null && search != undefined && search != "")
        TotalBillsMotionService.getSearchForCustomer(search).then(function (response) {
            $scope.searchItems = response.data;
            if ($scope.searchItems.length == 1) {
                $scope.search.customerName = $scope.searchItems[0].CUST_AR_NAME;
                $scope.search.CustID = $scope.searchItems[0].ACC_ID;
    ...
    $scope.addSelectedItm = function (itm) {
        $scope.search.customerName = itm.CUST_AR_NAME;
        $scope.search.CustID = itm.ACC_ID;
    ...
    $scope.onSelect = function (item, model, label) {
        ...
        $scope.search.CurrencyId = item.CURRENCY_ID;

        $scope.getCurrencyRate();
    };
    
    $scope.onSelectPayType = function (item, model, label) {
        $scope.search.payTypeId = item.PAY_TYPE_ID;
        $scope.search.payTypeName = item.PAY_TYPE_AR_NAME;
    };

    $scope.onSelectCompanyBranch = function (item, model, label) {
        $scope.search.branchId = item.COM_BRN_ID;
        $scope.search.COM_BRN_AR_NAME = item.COM_BRN_AR_NAME;
    };
    ...
    $scope.fnPrintReport = function () {
        ...
        var x1 = "تقرير حركة اجمالى الفواتير".split(" ").reverse().join(" ");
            ...
            content: [
                {
                    ...
                    table: {
                        ...
                        headerRows: 1,
                        //margin: [400, 400, 0, 0],
                        widths: $scope.reportOptionList[3].SELECTED && $scope.reportOptionList[2].SELECTED ? ['*', '*', '*', 'auto', 'auto', '*', 'auto', '*', '*', 'auto', 'auto'] :
                            $scope.reportOptionList[3].SELECTED ? ['*', '*', '*', 'auto', 'auto', '*', 'auto', '*', 'auto', 'auto'] :
                            $scope.reportOptionList[2].SELECTED ? ['*', '*', '*', 'auto', '*', '*', 'auto', '*', 'auto'] :
                            ['*', '*', '*', 'auto', 'auto', '*', 'auto', '*'],
    ...
    $scope.createArr = function (list) {
        ...
        if ($scope.reportOptionList[2].SELECTED && $scope.reportOptionList[3].SELECTED) {
            arr[0][10] = "المسلسل";
            arr[0][9] = "الفاتورة";
            arr[0][8] = "التاريخ";
            arr[0][7] = "الشرح";
        ...
        else if ($scope.reportOptionList[2].SELECTED) {
            arr[0][8] = "المسلسل";
            arr[0][7] = "الفاتورة";
            arr[0][6] = "التاريخ";
            arr[0][5] = "الشرح";
        ...
        else if ($scope.reportOptionList[3].SELECTED) {
            arr[0][9] = "المسلسل";
            arr[0][8] = "الفاتورة";
            arr[0][7] = "التاريخ";
            arr[0][6] = "الشرح";
        ...
        arr[0][3] = "العميل / المورد";
        arr[0][2] = "القيمة المدفوعة";
        arr[0][1] = "القيمة المؤجلة";
        arr[0][0] = "الاجمالى";
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    ...
                    if (key == "BILL_NUMBER") {
                        if ($scope.reportOptionList[2].SELECTED && $scope.reportOptionList[3].SELECTED) {
                            arr[index][9] = (obj["BILL_ABRV_AR"] + " - " + obj[key]).split(" ").reverse().join(" ");
                        }
                        else if ($scope.reportOptionList[2].SELECTED) {
                            arr[index][7] = (obj["BILL_ABRV_AR"] + " - " + obj[key]).split(" ").reverse().join(" ");
                        }
                        else if ($scope.reportOptionList[3].SELECTED) {
                            arr[index][8] = (obj["BILL_ABRV_AR"] + " - " + obj[key]).split(" ").reverse().join(" ");
                        }
                        else {
                            arr[index][6] = (obj["BILL_ABRV_AR"] + " - " + obj[key]).split(" ").reverse().join(" ");
                        }
                    }
                        ...
                        if ($scope.reportOptionList[2].SELECTED && $scope.reportOptionList[3].SELECTED) {
                            arr[index][8] = account_date;
                        }
                        else if ($scope.reportOptionList[2].SELECTED) {
                            arr[index][6] = account_date;
                        }
                        else if ($scope.reportOptionList[3].SELECTED) {
                            arr[index][7] = account_date;
                        }
                        else {
                        ...
                        if ($scope.reportOptionList[2].SELECTED && $scope.reportOptionList[3].SELECTED) {
                            arr[index][7] = obj[key] != null && obj[key] != undefined ? obj[key].split(" ").reverse().join(" ") : obj[key];
                        }
                        else if ($scope.reportOptionList[2].SELECTED) {
                            arr[index][5] = obj[key] != null && obj[key] != undefined ? obj[key].split(" ").reverse().join(" ") : obj[key];
                        }
                        else if ($scope.reportOptionList[3].SELECTED) {
                            arr[index][6] = obj[key] != null && obj[key] != undefined ? obj[key].split(" ").reverse().join(" ") : obj[key];
                        }
                        else {
                            arr[index][4] = obj[key] != null && obj[key] != undefined ? obj[key].split(" ").reverse().join(" ") : obj[key];
                        }
                    }

                    else if (key == "COM_STORE_AR_NAME") {
                        if ($scope.reportOptionList[2].SELECTED && $scope.reportOptionList[3].SELECTED) {
                            arr[index][6] = obj[key] != null && obj[key] != undefined ? obj[key].split(" ").reverse().join(" ") : obj[key];
                        }
                        else if ($scope.reportOptionList[2].SELECTED) {
                    ...
                    else if (key == "BILL_TOTAL_DISC") {
                        if ( $scope.reportOptionList[3].SELECTED) {
                            arr[index][5] = Number(obj[key]).toFixed(2);
                        }
                    }
                    else if (key == "BILL_TOTAL_EXTRA") {
                        if ($scope.reportOptionList[3].SELECTED) {
                            arr[index][4] = Number(obj[key]).toFixed(2);
                        }
                    }

                    else if (key == "ACC_AR_NAME") {

                        arr[index][3] = obj[key] != null && obj[key] != undefined ? obj[key].split(" ").reverse().join(" ") : obj[key];
                    }
                    ////////////////////////////
                    else if (key == "TotalPaid") {
                        arr[index][2] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "TotalRemaining") {

                        arr[index][1] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "BILL_TOTAL") {
