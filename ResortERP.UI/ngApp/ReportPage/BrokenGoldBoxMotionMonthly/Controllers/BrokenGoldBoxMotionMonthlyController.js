'use strict';
app.controller('BrokenGoldBoxMotionMonthlyController', ['$scope', '$rootScope', '$location', '$log', '$q', 'accountsService', 'dateFilter', '$filter', 'commonService', 'currencyService', '$base64', 'entryService', 'accountStatementGoldMonthlyService', 'compBranchesService', function ($scope, $rootScope, $location, $log, $q, accountsService, dateFilter, $filter, commonService, currencyService, $base64, entryService, accountStatementGoldMonthlyService, compBranchesService) {


    $scope.accountList = [];

    $scope.entryMasterList = [];
    

    $scope.cust_Acc_ID = null;

    $scope.startDate = "";
    $scope.endDate = "";

    $scope.show_print = false;
    $scope.var = { ACC_ID: null };

    $scope.UserBranches = [];
    $scope.dropdownSetting = {
        scrollable: true,
        scrollableHeight: '300px',
        checkBoxes: true,
        enableSearch: true,
        buttonClasses: 'btn btn-default btn-block',
        styleActive: true,
        smartButtonTextProvider: function (selectedItems) {
            if (selectedItems.length > 3) {
                return "تم اختيار " + selectedItems.length + ' فروع';
            }
            else if (selectedItems.length == 3) {
                return selectedItems[0].label + ', ' + selectedItems[1].label + ', ' + selectedItems[2].label;
            }
            else if (selectedItems.length == 2) {
                return selectedItems[0].label + ', ' + selectedItems[1].label;
            }
            else if (selectedItems.length == 1) {
                return selectedItems[0].label;
            }
            else {
                return 'اختار فرع';
            }
        }
    }

    $scope.getAccountList = function () {        
        accountsService.GetAccountsFilteredByType("GoldBox").then(function (result) {
            $scope.accountList = result.data;
        }, function (error) {
            console.log(error.data.message);
        });       
    }

    $scope.accountStatmentGoldPageLoad = function () {

        $scope.getAccountList();
        $scope.GetUserBranches();

        $scope.var.ACC_ID = 775;
        $scope.startDate = new Date(2018, 8, 14);
        $scope.endDate = new Date();
    }

    $scope.GetUserBranches = function () {
        compBranchesService.getUserBranches().then(function (response) {
            if (response.data != undefined && response.data.length > 0) {
                angular.forEach(response.data, function (value, index) {
                    $scope.UserBranches.push({ id: value.COM_BRN_ID, label: value.COM_BRN_AR_NAME });
                });
                $scope.SelectedBranch = [$scope.UserBranches[0]];
            }
        }, function (error) { console.log(error.data.message); });
    };
 
    $scope.GetSearchResult = function () {

        //accountStatementService.getByID($scope.cust_Acc_ID).then(function (results) {

        //    console.log(results.data);

        //    $scope.entryMasterList = results.data;
        //});

        //var Accounts = '<?xml version ="1.0" encoding="windows-1256"?> <DS><ACCOUNTS><ACC_ID>775</ACC_ID></ACCOUNTS><ACCOUNTS><ACC_ID>54</ACC_ID></ACCOUNTS></DS>';

        //var Accounts = "<DS><ACCOUNTS><ACC_ID>775</ACC_ID></ACCOUNTS><ACCOUNTS><ACC_ID>775</ACC_ID></ACCOUNTS><ACCOUNTS><ACC_ID>54</ACC_ID></ACCOUNTS><ACCOUNTS><ACC_ID>3385</ACC_ID></ACCOUNTS></DS>";

        if ($scope.var.ACC_ID && $scope.startDate && $scope.endDate && $scope.SelectedBranch.length > 0) {

            debugger;
            var companyBranches = '';
            angular.forEach($scope.SelectedBranch, function (item, index) {
                companyBranches += item.id + ',';
            })
            companyBranches = companyBranches.substr(0, companyBranches.length - 1);

            var Accounts = "<DS><ACCOUNTS><ACC_ID>" + $scope.var.ACC_ID + "</ACC_ID></ACCOUNTS></DS>";

            var d1 = new Date($scope.startDate);

            var startDate = d1.getFullYear() + "" + ("0" + (d1.getMonth() + 1)).slice(-2) + "" + ("0" + d1.getDate()).slice(-2);

            var d2 = new Date($scope.endDate);

            var endDate = d2.getFullYear() + "" + ("0" + (d2.getMonth() + 1)).slice(-2) + "" + ("0" + (d2.getDate())).slice(-2);
            var arEndDate = $scope.ConvertAllDate($scope.endDate);
            var arStartDate = $scope.ConvertAllDate($scope.startDate);
            var ReportName = "getAccountStatementGoldMonthlyReport";
            //ASPX page URL to load report  
            var src = '../Reports/ReportForm/ReportPage.aspx?';
            //We can add parameters here  
            src = src + "ReportName=" + ReportName + "&companyBranches=" + companyBranches + "&Accounts=" + $scope.var.ACC_ID + "&startDate=" + startDate + "&endDate=" + endDate + "&arStartDate=" + arStartDate + "&arEndDate=" + arEndDate;

            window.open(src,"_blank");
            
            var iframe = '<iframe id="reportFrame" width="100%" height="800px" scrolling="no" frameborder="0" src="' + src + '" allowfullscreen></iframe>';
            $("#divReport").html(iframe); 

            //accountStatementGoldMonthlyService.getReportResult(companyBranches, Accounts, startDate, endDate).then(function (results) {
                
            //    $scope.entryMasterList = results.data;
            //    $scope.show_print = true;
            //});

        }

        else swal("عفوا", " لابد من البحث بالحساب و التاريخ والفرع ", "error");

    }

    


    $scope.clearEnity = function () {

        //d.getFullYear() + "" + (d.getMonth() + 1) + "" + d.getDate();

        $scope.entryMasterList = [];
        
        $scope.var.ACC_ID = null;

        $scope.startDate = "";
        $scope.endDate = "";

        $scope.show_print = false;

    }

    $scope.fnExcelReport = function () {



        console.log($scope.createArr($scope.entryMasterList));

        //var client_name = $("#cust_Acc_ID option:selected").text();

        var client_name = $("#cust_Acc_ID").text().split("\n")[1].trim();

        var arabic = /[\u0600-\u06FF]/;

        var x1 = "حركة صندوق الذهب الكسر (شهرية )".split(" ").reverse().join(" ");

        var x2 = ("اسم الصندوق : " + client_name).split(" ").reverse().join(" ");
        var dd1 = new Date($scope.startDate);
        var dd2 = new Date($scope.endDate);


        var FristDate = $scope.ConvertAllDate(dd1);
        var LastDate = $scope.ConvertAllDate(dd2);
        if (arabic.test(client_name) == false) {

            x2 = ("اسم الصندوق : ").split(" ").reverse().join(" ");

            x2 = client_name + " " + x2;
        }



        pdfMake.fonts = {
            custom: {
                normal: 'DroidKufi-Regular.ttf'
            }
        }

        var docDefinition = {
            content: [
                {
                    //layout: 'lightHorizontalLines', // optional
                    layout: {
                        hLineWidth: function (i, node) {

                            if (i === 0 || i === node.table.body.length) {
                                return 1;
                            }
                            return (i === node.table.headerRows) ? 2 : 1;
                        },
                        vLineWidth: function (i) {
                            return 1;
                        },
                        hLineColor: function (i) {
                            return i === 1 ? 'black' : '#aaa';
                        },
                        paddingLeft: function (i) {
                            return i === 0 ? 0 : 8;
                        },
                        paddingRight: function (i, node) {
                            return (i === node.table.widths.length - 1) ? 0 : 8;
                        },
                        fillColor: function (i, node) {

                            if (i == 1) {
                                return 'orange';
                            }
                            else {
                                return i % 2 == 0 ? 'white' : '#eee';
                            }

                        }
                    },
                    table: {
                        // headers are automatically repeated if the table spans over multiple pages
                        // you can declare how many rows should be treated as headers
                        //widths: [60, 60, 160, 80, 80],

                        headerRows: 1,
                        widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto'],
                        //margin: [400, 400, 0, 0],
                        body: $scope.createArr($scope.entryMasterList)
                    }
                }
            ],
            images: {
                bee: "data:image/png;base64,"
                   },
            pageMargins: [250, 150, 100, 100],
            pageOrientation: "landscape",
            pageSize: 'A3',
            header: {
                text: client_name + "  /  " + x1 + "\n" + LastDate + "   " + "  تاريخ حتي   " + FristDate + "   " + " يوم من للفترة  ",
                alignment: "center",
                margin: [0, 80, 0, 0],
                fontSize: 16,
            },
            background: function (page) {

                return [
                    '\r\n',
                    {
                        image: 'bee',
                        width: 200
                    }
                ];

            },
            defaultStyle: {
                font: 'custom',
                alignment: 'center',
                top: 200
            },
        };

        pdfMake.createPdf(docDefinition).download();


    }

    $scope.createArr_ = function (list) {


        var arr = [];

        for (var i = 0; i < list.length; i++) {
            arr[i] = [];
        }

        for (var i = 0; i < list.length; i++) {

            var obj = list[i];

            var j = 0;
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (j >= 5) {
                        continue;
                    }
                    //if (j == 4) {
                    arr[i][j] = obj[key].split(" ").reverse().join(" ");
                    //}
                    //else
                    //    arr[i][j] = obj[key];
                    j++;
                }
            }
        }
        return arr;
    }

    $scope.createArr = function (list) {
        debugger;
        var arr = [];
        for (var i = 0; i <= list.length; i++) {
            arr[i] = [];
        }

        arr[0][0] = "الرصيد 24";
        arr[0][1] = "دائن 24";
        arr[0][2] = "مدين 24";

        arr[0][3] = "الرصيد 22";
        arr[0][4] = "دائن 22";
        arr[0][5] = "مدين 22";

        arr[0][6] = "الرصيد 21";
        arr[0][7] = "دائن 21";
        arr[0][8] = "مدين 21";

        arr[0][9] = "الرصيد 18";
        arr[0][10] = "دائن 18";
        arr[0][11] = "مدين 18";
        arr[0][12] = "الشهر";

        for (var i = 0; i < 12; i++) {
            arr[0][i] = arr[0][i].split(" ").reverse().join(" ");
        }

        var index = 1;
        for (var i = 0; i < list.length; i++) {
            var obj = list[i];
            arr[index][12] = i + 1;
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (key == "BAL_Gold24") {
                        arr[index][0] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "CREDIT_Gold24") {
                        arr[index][1] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "DEBIT_Gold24") {
                        arr[index][2] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "BAL_Gold22") {
                        arr[index][3] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "CREDIT_Gold22") {
                        arr[index][4] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "DEBIT_Gold22") {
                        arr[index][5] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "BAL_Gold21") {
                        arr[index][6] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "CREDIT_Gold21") {
                        arr[index][7] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "DEBIT_Gold21") {
                        arr[index][8] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "BAL_Gold18") {
                        arr[index][9] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "CREDIT_Gold18") {
                        arr[index][10] = Number(obj[key]).toFixed(2);
                    }
                    else if (key == "DEBIT_Gold18") {
                        arr[index][11] = Number(obj[key]).toFixed(2);
                    }
                  
                    else if (key == "DATE") {
                        if (obj[key]) {
                            //var d1 = new Date(obj[key]);
                            //var account_date = d1.getFullYear() + "-" + ("0" + (d1.getMonth() + 1)).slice(-2) + "-" + ("0" + d1.getDate()).slice(-2);
                            arr[index][12] = $scope.ConvertDate(obj[key]); //account_date
                        }
                        else {
                            arr[index][12] = "";
                        }
                    }
                }
            }
            index++;
        }
        debugger;
        var FristHeader = [
            { text: ' 24 عيار ', style: ' tableHeader: {bold: true, fontSize: 13,color}', colSpan: 3, alignment: 'center' }, '','', 
            { text: '22 عيار', style: 'tableHeader: {bold: true, fontSize: 13,color}', colSpan: 3, alignment: 'center' }, '', '', 
            { text: '21 عيار', style: 'tableHeader: {bold: true, fontSize: 13,color}', colSpan: 3, alignment: 'center' }, '', '', 
            { text: ' 18 عيار', style: 'tableHeader: {bold: true, fontSize: 13,color}', colSpan: 3, alignment: 'center' }, '', '', 
           
            { text: 'البيان', style: 'tableHeader: {bold: true, fontSize: 13,color}', colSpan: 1, alignment: 'center' }
           

        ];
        var HeaderFrist = arr.unshift(FristHeader);

        return arr;
    }

    $scope.ConvertDate = function (DATE) {
        debugger;
        if (DATE != null) {

            DATE = '1-' + DATE;
            var dateParts = DATE.split("-");
            var dateObject = new Date(+dateParts[2], dateParts[1] - 1);

            var months = ["يناير", "فبراير", "مارس", "إبريل", "مايو", "يونيو",
                "يوليو", "أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"
            ];
            var days = ["اﻷحد", "اﻷثنين", "الثلاثاء", "اﻷربعاء", "الخميس", "الجمعة", "السبت"];
            var delDateString = months[dateObject.getMonth()] + ', ' + dateObject.getFullYear();

            return delDateString;
        }
        return null;

    }
    $scope.ConvertAllDate = function (DATE) {
        if (DATE != null) {


            var months = ["يناير", "فبراير", "مارس", "إبريل", "مايو", "يونيو",
                "يوليو", "أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"
            ];
            var days = ["اﻷحد", "اﻷثنين", "الثلاثاء", "اﻷربعاء", "الخميس", "الجمعة", "السبت"];
            var delDateString = DATE.getFullYear() + ' ' + days[DATE.getDay()] + ' ' + DATE.getDate() + ' ' + months[DATE.getMonth()] ;

            return delDateString;
        }
        return null;

    }

    $scope.GetValue = function (Credit, debit,gram,index)
    {
        if (parseFloat(Credit) == undefined || parseFloat(Credit) == null || parseFloat(Credit) == 0) {
            Credit = 0;
        }
        if ((debit) == undefined || (debit) == null || parseFloat(debit) == 0) {
            debit = 0;
        }
        if (parseFloat(Credit) - parseFloat(debit) < 0) {
            if (gram == 24) {

                return '(' + ((parseFloat(Credit) - parseFloat(debit)) * -1) + ')';
            }
            else if (gram == 22) {

                return '(' + ((parseFloat(Credit) - parseFloat(debit)) * -1) + ')';
            }
            else if (gram == 21) {
                var cols = document.getElementsByClassName('color21');
                $(cols[index - 1]).css("color", "red");
                // $(cols[index - 1]).css("backgroundColor", "red");
                var colstotal = document.getElementsByClassName('totalclass21');
                $(colstotal[index - 1]).css("color", "red");
               // $(colstotal[index - 1]).css("backgroundColor", "red");
                

                return '(' + ((parseFloat(Credit) - parseFloat(debit)) * -1) + ')';
            }
            else if (gram == 18) {

                return '(' + ((parseFloat(Credit) - parseFloat(debit)) * -1) + ')';
            }

        }

        else
        {
            var cols = document.getElementsByClassName('color21');
            $(cols[index - 1]).css("Color", "white");
            var colstotal = document.getElementsByClassName('totalclass21');
            $(colstotal[index - 1]).css("Color", "#f1f2f6");
            return (parseFloat(Credit) - parseFloat(debit));
        }
    }
       
    $scope.GetSumDebit = function (key, index) {
        let val = 0;
        debugger;
        if (index != 0) {
            for (var i = 0; i <= index; i++) {
                if (key == "DEBIT_Gold24")
                {
                    val += $scope.entryMasterList[i].DEBIT_Gold24;
                }
                else if (key == "DEBIT_Gold22")
                {

                    val += $scope.entryMasterList[i].DEBIT_Gold22;
                }
                else if (key == "DEBIT_Gold21")
                {

                    val += $scope.entryMasterList[i].DEBIT_Gold21;
                }
                else if (key == "DEBIT_Gold18")
                {

                    val += $scope.entryMasterList[i].DEBIT_Gold18;
                }
            }

            return val;
        }

        else
            return 0;

    }
    $scope.GetSumCredit = function (key, index) {
        let val = 0;
        if (index != 0) {
            for (var i = 0; i <= index; i++) {
                if (key == "CREDIT_Gold24") {
                    val += $scope.entryMasterList[i].CREDIT_Gold24;
                }
                else if (key == "CREDIT_Gold21") {

                    val += $scope.entryMasterList[i].CREDIT_Gold21;
                }
                else if (key == "CREDIT_Gold22") {

                    val += $scope.entryMasterList[i].CREDIT_Gold22;
                }
                else if (key == "CREDIT_Gold18") {

                    val += $scope.entryMasterList[i].CREDIT_Gold18;
                }
            }

            return val;
        }
        else
            return 0;


    }
    

}]);


